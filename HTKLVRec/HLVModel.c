...
StateInfo_lv *ConvertHSet(MemHeap *heap, HMMSet *hset, Boolean useHModel)
{
   ...
   if (!useHModel) {
      ...
      if(hset->TVWR)
    	  HLVMODEL_BLOCK_STE_OFFSET(si) = HLVMODEL_BLOCK_INVVAR_OFFSET(si) + si->nVec * HLVMODEL_VEC_PAD;
      ...
      while (GoNextState (&hss, FALSE)) {
         ...
         if(hset->TVWR)
        	 (*((StreamElem **) ((base) + HLVMODEL_BLOCK_STE_OFFSET(si))))=se;
      } 
      EndHMMScan (&hss);
   }
   else
...
LogFloat OutP_lv (StateInfo_lv *si,  unsigned short s, float *x, LogFloat* mixOutP, int id, int block, int block_i)
{
   int m, i;
   LogDouble bx;
   LogFloat px;
   ...
   if(hset->fMPE){
 	  x=ApplyFprojection((HDFInfo*)hset->hdfInfo,id);
   }
   if(hset->TVWR){
	   ste = (*((StreamElem **) ((base) + HLVMODEL_BLOCK_STE_OFFSET(si))));
	   ApplyWprojection(ste, (HDFInfo*)hset->hdfInfo, id, 1);
   }
   for (m = 1; m <= nMix; m++) {
	  swt = HLVMODEL_BLOCK_MIXW(si,base);
	  wt=hset->TVWR?MixLogWeight(hset,(ste->spdf.cpdf+m)->weight):swt;
      px = HLVMODEL_BLOCK_GCONST(si,base);
      mp = HLVMODEL_BLOCK_MPDF(si,base);
      if(swt>LMINMIX){
          for (i = 0; i < si->nDim; ++i) {
             xmm = x[i] - mean[i];
             px += xmm*xmm * invVar[i];
      ...
      if(mixOutP&&mp->mIdx>0){
    	  mixOutP[mp->mIdx*block+block_i]=swt>LSMALL?swt+px:LZERO;
      }
                ...
                int n, int sIdx, float acScale, LogFloat *outP, LogFloat *mixOutP, int id)
{
   ...
   for (i = 0; i < n; ++i) {
      outP[i] = OutP_lv (si, sIdx, &obsBlock[i]->fv[1][1], mixOutP, id+i, n, i);
   }
   /* acoustic scaling */
   if (acScale != 1.0){
      for (i = 0; i < n; ++i){
