...
static char * pcfn = NULL;	 /* phone cluster definition file*/
static int* pcmap=NULL;
...
static int nnmix=1;	/*default, num of noise mixtures, 1, no compensation or regular one mix compensation*/
...
void DoRecognition (DecoderInst *dec, char *fn, HLink* gmmcluster);
...
main (int argc, char *argv[])
{
   ...
   while (NextArg () == SWITCHARG) {
	 ...
	 break;
      ...
      case 'n':
    	  if(!strcmp(s,"nnmix")){
    		  if(NextArg()!=INTARG)
    			  HError(1,"HDecode: num of noise mixtures expected");
    		  nnmix=GetChkedInt(0, 1024,s);break;
    	  }
    	  else if(strlen (s) != 1){
	 ...
	 break;
      case 'Y':
    	 if (NextArg()!=STRINGARG)
    		HError(2319,"HDecode: -Y: Posterior directory expected");
    	 posDir = GetStrArg();
    	 if (NextArg()==STRINGARG)
    		 posExt = GetStrArg();
    	 break;
      case 'W':
    	 if(NextArg()!=STRINGARG)
    		 HError(2319,"HERest: -W: gmmcluster definition file expected");
    	 AddMMF(&synrhset,GetStrArg());
    	 gotgmmcluster=TRUE;
    	 break;
      case 'e':
    	 if(NextArg()!=STRINGARG)
    		 HError(2319,"HERest: -e: gmmcluster class list file expected");
   ...
   while (NumArgs () > 0) {
      ...
      DoRecognition (dec, datafn, gmmcluster);
      /* perform recognition */
   }
...
datafn=fn;
   ...
   if(hset.TVWR){
  	 ...
  	 if(gmmcluster&&((xfInfo.inXForm&&xfInfo.inXForm->xformSet->numXForms>1)||hset.xf)){
  		 HError(1,"Main: Current posterior synthesiser only support one linear transformation, **non input xform version**");
  	 }
  	 LoadHDF4TVWR(&dec->hdfInfo, dff, posfn, datafn, &synrhset, gmmcluster, FALSE, xfInfo.inXForm?xfInfo.inXForm->xformSet->xforms[1]:NULL,nnmix, "");
   }
   if(hset.fMPE){
  	 if(gmmcluster&&(xfInfo.inXForm->xformSet->numXForms>1||hset.xf)){
  		 HError(1,"Main: Current posterior synthesiser only support one linear transformation, **non input xform version**");
  	 }
  	 LoadHDF4fMPE(&dec->hdfInfo, dff, posfn, datafn, &synrhset, gmmcluster, hset.TVWR, xfInfo.inXForm?xfInfo.inXForm->xformSet->xforms[1]:NULL);
   }
   startClock = clock();
