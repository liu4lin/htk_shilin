...
static Boolean disableGMM=FALSE;
static Boolean disableTVWR=FALSE;

/* -------------------------- Prototypes -------------------------------- */
void SetConfParms (void);
void ReportUsage (void);
DecoderInst *Initialise (void);
double DoRecognition (DecoderInst *dec, char *fn);
...
ReportUsage (void)
{
   ...
   printf (" -Y s    posterior file directory            none\n");
   printf (" -Z s    posterior file extension            pos\n");
   printf (" -W s    posterior synthesiser MMF file      none\n");
   printf (" -e s    synthesiser MMF phone list          none\n");
   printf (" -nnmix i num of noise mixtures              1\n");
...
int main (int argc, char *argv[])
{
   ...
   CreateHMMSet(&nhset,&modelHeap,TRUE);
   CreateHMMSet(&synrhset,&modelHeap,TRUE);
	 ...
	 HError (4019, "HDecode: Bad switch %s; must be single letter", s);*/
      ...
      case 'n':
    	  if (!strcmp(s,"nList")){
    		  ...
    		  HError (4019, "HDecode: Unknown switch %s", s);break;
    	  }
      ...
      case 'W':
    	 if(NextArg()!=STRINGARG)
    		 HError(2319,"HDecode: -W: Posterior synthesiser model file expected");
      ...
      case 'e':
    	 if(NextArg()!=STRINGARG)
    		 HError(2319,"HDecode: -e: Posterior synthesiser list file expected");
   ...
   dec = Initialise ();
   if(NAT){
	   mus_x=CreateMatrix(&gstack,hset.numMix,hset.vecSize);
	   cvs_x=CreateMatrix(&gstack,hset.numMix,hset.vecSize);
	   HMMScanState hss;
	   MixPDF* mp;
	   int mixIdx=0;
	   NewHMMScan(&hset,&hss);
	   while(GoNextMix(&hss,FALSE)){
			...
			CopyVector(mp->mean,mus_x[mixIdx]);
			if(hss.mp->ckind==DIAGC){
				CopyVector(mp->cov.var,cvs_x[mixIdx]);
			}
			if(hss.mp->ckind==INVDIAGC){
				CopyiVector(mp->cov.var,cvs_x[mixIdx]);
			}
	   }
	   EndHMMScan(&hss);
	   if(useGmmSynr){
		   lmus_x=CreateMatrix(&gstack,synrhset.numMix,synrhset.vecSize);
		   ...
		   while(GoNextMix(&hss,FALSE)){
			   ...
			   lcvs_x[mixIdx]=CreateMatrix(&gstack,synrhset.vecSize,synrhset.vecSize);
   ...
   while (NumArgs () > 0) {
      ...
      if(NAT){
    	  char id[32];
    	  BaseOf(datafn, id);
    	  if(nlistWrite||lnlistWrite){
    		  /*estimate noise model*/
    		  iter=nlistWrite?CreateNoiseModel(&gstack, DIAGC, hset.vecSize, id, &nlistTail):iter;
    		  liter=lnlistWrite?CreateNoiseModel(&gstack, FULLC, synrhset.vecSize, id, &lnlistTail):liter;
    		  EstimateNoiseModel(nlistWrite?iter:NULL, lnlistWrite?liter:NULL, hset.vecSize, synrhset.vecSize, datafn, trimNum);
    	  }
    	  ...
    	  /*compensate HMM parameters*/
    	  VtsAdapt(&hset, iter->n_mean, iter->h_mean, iter->cov.var, mus_x, cvs_x);
    	  /*compensate synthesizer parameters*/
    	  if(useGmmSynr){
    		  lVtsAdapt(&synrhset, liter->n_mean, liter->h_mean, liter->cov.inv, lmus_x, lcvs_x, NULL);
    	  }
    	  if(!nlistWrite){
    		  iter=nlist?iter->next:NULL;
    		  liter=lnlist?liter->next:NULL;
    	  }
    	  /*FreeDVector(&gstack,mu_n);*/
      }
      totalCpuSec+=DoRecognition (dec, datafn);
      totalFrames+=dec->frame;
      if(hset.TVWR){
    	  ClearRwgtCache(&hset);
      }
      /* perform recognition */
   ...
   printf ("Total CPU time %f, total frames %f, total utterance length %f, RT factor %f\n",
		   totalCpuSec, totalFrames, totalFrames*dec->frameDur, totalCpuSec / (totalFrames*dec->frameDur));
   fflush(stdout);
...
DecoderInst *Initialise (void)
{
   ...
   /*hset.disableGMM=disableGMM;
   hset.TVWR=!disableTVWR;
   /* convert to INVDIAGC */
   ConvDiagC (&hset, TRUE);
   ConvLogWt (&hset);
   ...
   if(hset.fMPE||hset.TVWR){
	   if (outpBlocksize != 1) {
		   HError (-4019, "HDecode: outP blocksize >1 not supported with fMPE and TVWR code! setting to 1.");
		   outpBlocksize = 1;
	   }
   }
   ...
   dec = CreateDecoderInst (&hset, lm, nTok, TRUE, useHModel, outpBlocksize,
                            ...
                            modAlign);
   if(dnn_dim>0){
	   hset.vecSize=dnn_dim;
	   hset.swidth[1]=dnn_dim;
   }
...
double DoRecognition (DecoderInst *dec, char *fn)
{
   char buf1[MAXSTRLEN], buf2[MAXSTRLEN],ext[12];
   ParmBuf parmBuf;
   BufferInfo pbInfo;
   int frameN, frameProc, i, bs, pos_stream;
   Transcription *trans;
   Lattice *lat;
   ...
   char *datafn, posfn[MAXSTRLEN];
   datafn=fn;
   int dnn_row,dnn_col,dnn_i=0;
   FILE *dnn_f=NULL;
   char dnn_buf[32];
   ...
   { 
      ...
      if(NAT&&xfInfo.useInXForm ) {/*after the VTS compensation, in case of model adaptation such as MLLRMEAN*/
   ...
   startClock = clock();/*including the posterior loading or synthesis time*/
   if(hset.TVWR){
	 for(pos_stream=1;pos_stream<=numposdirs;pos_stream++){
    	 ...
    	 LoadHDF4TVWR(&dec->hdfInfo, dff, posDir[pos_stream], posExt, datafn, &synrhset, phnSynr, xfInfo.inXForm?xfInfo.inXForm->xformSet->xforms[1]:NULL, pos_stream);
	 }
   }
   if(hset.fMPE){
	 if(useTiedStatePos){
		 LoadHDF4fMPE2(&dec->hdfInfo, dff, datafn, tspSynr, hset.TVWR);
	 }
	 else{
		 LoadHDF4fMPE(&dec->hdfInfo, dff, posfn, datafn, &synrhset, phnSynr, hset.TVWR, xfInfo.inXForm?xfInfo.inXForm->xformSet->xforms[1]:NULL);
	 }
   }
   ...
   if(dnn_dim){/*sparse posteriors*/
	   pbInfo.tgtSampRate=1.0e5;
	   if((dnn_f=fopen(fn,"r"))==NULL){
		   HError(1,"Cannot open file: %s",fn);
	   }
	   fscanf(dnn_f,"%d %d\n",&dnn_row,&dnn_col);
   }
   else{
   ...
   frameN = frameProc = 0;
   while (dnn_dim?dnn_i<dnn_row:(BufferStatus (parmBuf) != PB_CLEARED)) {
	  if(dnn_dim){
		  dnn_i+=outpBlocksize;
		  int idx,i,j;
		  char ch;
