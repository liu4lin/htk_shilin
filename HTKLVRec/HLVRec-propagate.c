...
#define NBINS 64
         ...
         if (nTok == dec->nTok) {
            ...
            if (nBetter > dec->nTok) {  /* multiple tokens with delta == limit  ==> delete some */
               for (i = 0; nBetter > dec->nTok&&i < nWinTok; ++i)
                  if (winTok[i].delta == limit) {
...
static void PropagateInternal (DecoderInst *dec, LexNodeInst *inst)
{
   ...
   if (hmm->tIdx < 0) {
      ...
      /*char *name=HMMPhysName(dec->hset,hmm);*/
