...
static char * posDir = NULL;	 /* directory to store posterior files h_t */
static char * posDir2 = NULL;	 /* directory to store posterior files q_t*/
static char * posExt = NULL;	 /* extension of posterior files */

static HLink* phnSynr=NULL;
static char synrListFn[MAXFNAMELEN];
...
static Matrix lx_mu_bk=NULL;
static Matrix* lx_cv_bk=NULL;
...
static Boolean useGmmSynr=FALSE;
...
int main(int argc, char *argv[])
{
   ...
   while (NextArg() == SWITCHARG) {
	 ...
	 break;
      ...
      case 'N':
         ...
         HError (1, "HVite: Unknown switch %s",s);
         break;
      case 'n':
    	  if (!strcmp(s,"nList")){
    		  nlist=GetStrArg();
	 ...
	 if (NextArg() != SWITCHARG)
      ...
      case 'Y':
    	 if(!strcmp(s,"Y2")){
        	 ...
        	 if (NextArg()!=STRINGARG)
        		HError(2319,"HVite: -Y: Posterior directory expected");
        	 posDir = GetStrArg();
        	 if (NextArg()==STRINGARG)
        	     posExt = GetStrArg();
    	 }
      ...
      case 'W':
    	 if(NextArg()!=STRINGARG)
    		 HError(2319,"HVite: -W: gmmcluster definition file expected");
    	 AddMMF(&synrhset,GetStrArg());
    	 useGmmSynr=TRUE;
    	 break;
      case 'e':
    	 if(NextArg()!=STRINGARG)
    		 HError(2319,"HVite: -e: gmmcluster class list file expected");
   ...
   Initialise();
   if(NAT){
	   x_mu_bk=CreateMatrix(&gstack,hset.numMix,hset.vecSize);
	   x_cv_bk=CreateMatrix(&gstack,hset.numMix,hset.vecSize);
	   HMMScanState hss;
	   MixPDF* mp;
	   int mixIdx=0;
	   NewHMMScan(&hset,&hss);
	   ...
	   EndHMMScan(&hss);
	   if(useGmmSynr){
		   lx_mu_bk=CreateMatrix(&gstack,synrhset.numMix,synrhset.vecSize);
		   lx_cv_bk=(Matrix*)New(&gstack,(synrhset.numMix+1)*sizeof(Matrix));
		   HMMScanState hss;
		   MixPDF* mp;
		   int mixIdx=0;
		   ...
		   while(GoNextMix(&hss,FALSE)){
			   ...
			   mixIdx=mp->mIdx;
			   if(mixIdx<=0)continue;
			   CopyVector(mp->mean,lx_mu_bk[mixIdx]);
			   lx_cv_bk[mixIdx]=CreateMatrix(&gstack,synrhset.vecSize,synrhset.vecSize);
			   CovInvert(&gstack, mp->cov.inv,lx_cv_bk[mixIdx]);/*switch to variance*/
		   }
		   EndHMMScan(&hss);
	   }
   }
...
Boolean ProcessFile(char *fn, Network *net, int utterNum, LogDouble currGenBeam, Boolean restartable)
{
   ...
   if(NAT){
 	  ...
 	  if(nlistWrite||lnlistWrite){
 		  ...
 		  EstimateNoiseModel(nlistWrite?iter:NULL, lnlistWrite?liter:NULL, hset.vecSize, synrhset.vecSize, fn, trimNum);
 	  }
 	  ...
 	  if(useGmmSynr){
 		  lVtsAdapt(&synrhset, liter->n_mean, liter->h_mean, liter->cov.inv, lx_mu_bk, lx_cv_bk, NULL);
 	  }
 	  if(!nlistWrite){
   ...
   if(hset.TVWR){
  	 /*using non inputxform can let us just keep one MMF and multiple one global xform for each speaker,
  	  * otherwise, we have to store one MMF for each speaker */
  	 ...
  	 LoadHDF4TVWR(vri->hdfInfo, dff, posDir, posExt, datafn, &synrhset, phnSynr, xfInfo.inXForm?xfInfo.inXForm->xformSet->xforms[1]:NULL, 1);
   }
   if(hset.fMPE){
  	 if(phnSynr&&(xfInfo.inXForm->xformSet->numXForms>1||hset.xf)){
  		 HError(1,"Main: Current posterior synthesiser only support one linear transformation, **non input xform version**");
  	 }
  	 LoadHDF4fMPE(vri->hdfInfo, dff, posfn, datafn, &synrhset, phnSynr, hset.TVWR, xfInfo.inXForm?xfInfo.inXForm->xformSet->xforms[1]:NULL);
   }
   tact=0;nFrames=0;
   if(!dnn_dim)
	   StartBuffer(pbuf);
   while(dnn_dim?dnn_i<dnn_row:BufferStatus(pbuf)!=PB_CLEARED) {
	   if(dnn_dim){
		   dnn_i++;
		   int idx,j;
		   char ch;
...
void DoAlignment(void)
{
   ...
   while (NumArgs()>0) {
      ...
      if (loadNetworks) {
         ...
         if(wdNet==NULL){
        	printf("Creating Lattice failed, skip it\n");
        	continue;
         }
         if (trace&T_TOP) {
	 ...
	 */
	 incXForm = CreateAdaptXForm(&hset,"inc");
         TidyBaseAccs();
	 GenAdaptXForm(&hset,incXForm,NULL);
