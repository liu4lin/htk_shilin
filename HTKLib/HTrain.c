...
void SequenceCov(Sequence ss, CovKind ck, Covariance cov, Vector mean)
{
   ...
   case FULLC:    /* inverse full covariance matrix */
      ...
      break;
   default:
      HError(7170,"SequenceCov: unsupported cov kind [%d]",ck);break;
   }
}
...
static float Distance(Vector v1, Vector v2)
{
   ...
   case FULLC:
      ...
      break;
   default:
      HError(7170,"Distance: bad cov kind %d",dck);break;
   }
   if (trace&T_CDI) {
...
static void FindCovariance(int n)
{
   ...
   case FULLC:    /* inverse full covariance matrix */
      ...
      for (j=1; j<=vSize; j++)
         for (k=1; k<=j; k++)
            t[j][k] = xsum[j][k]/nx;
      CovInvert(&gstack, t,c); Mat2Tri(c,t);
      FreeDMatrix(&gstack,xsum);
      break;
   default:
      HError(7170,"FindCovariance: unsupported cov kind [%d]",ccs->ck);break;
   }
}
                           ...
                           Boolean treeCluster, CovKind distck, CovKind clusck, Covariance distcov)
{
   ...
   for (i=1; i<=numClust; i++){
      ...
      case FULLC:
         ...
         break;
      default:
         HError(7170,"InitClustering: bad cluster ckind");break;
      }
      ccs->cl[i].cov = cov;
   }
...
static int muC,vaC,trC,rwtC,wtC,prC,cC;

/* CreateDAcc:  create an accumulator for differential */
static DAcc *CreateDAcc(MemHeap *x, int rows, int cols, int nPara)
{
   DAcc *dAcc;/*an array of ma, for num, den, prior, etc. nPara is number of different individual case*/
   int count;
   dAcc = (DAcc *)New(x,sizeof(DAcc)*nPara);
   for(count=0;count<nPara;count++){
	   dAcc[count].rows=rows;
	   dAcc[count].pGradient=CreateMatrix(x,rows,cols);
	   dAcc[count].nGradient=CreateMatrix(x,rows,cols);
	   dAcc[count].sqSum=CreateMatrix(x,rows,cols);
	   dAcc[count].sumSq=CreateMatrix(x,rows,cols);
	   dAcc[count].check1=CreateMatrix(x,rows,2);
	   dAcc[count].check2=CreateMatrix(x,rows,2);
	   ZeroMatrix(dAcc[count].pGradient);
	   ZeroMatrix(dAcc[count].nGradient);
	   ZeroMatrix(dAcc[count].sqSum);
	   ZeroMatrix(dAcc[count].sumSq);
	   ZeroMatrix(dAcc[count].check1);
	   ZeroMatrix(dAcc[count].check2);
  }
  return dAcc;
}

/* CreateMuAcc:  create an accumulator for means */
static MuAcc *CreateMuAcc(MemHeap *x, int vSize, int nPara, Boolean storedif, Boolean upPseudoAM, Boolean upPseudoLAM)
{
   MuAcc *ma;/*an array of ma, for num, den, prior, etc. nPara is number of different individual case*/
   int count,i;
   ma = (MuAcc *)New(x,sizeof(MuAcc)*nPara);
   for(count=0;count<nPara;count++){
     ma[count].mu = CreateVector(x,vSize);
     ZeroVector(ma[count].mu);
     if(storedif&&count==nPara-1){
    	 ma[count].udif=CreateVector(x,vSize);
    	 ZeroVector(ma[count].udif);
     }
     else{
    	 ma[count].udif=NULL;
     }
     if(upPseudoAM){
    	 ma[count].GtPG=(TriMat*)New(x,sizeof(TriMat)*3);
    	 ma[count].GtPacc1=(Vector*)New(x,sizeof(Vector)*3);
    	 for(i=0;i<3;i++){
    		 ma[count].GtPG[i]=CreateTriMat(x,vSize/3);
    		 ma[count].GtPacc1[i]=CreateVector(x,vSize/3);
    		 ZeroTriMat(ma[count].GtPG[i]);
    		 ZeroVector(ma[count].GtPacc1[i]);
    	 }
    	 ma[count].acc_k=(NATAcc*)New(x,sizeof(NATAcc));/*create it as a header of list*/
    	 ma[count].acc_k->occ_k=0;
    	 ma[count].acc_k->mu_k=NULL;
    	 ma[count].acc_k->cov_k.var=NULL;
    	 ma[count].acc_k->noise_k=NULL;
    	 ma[count].acc_k->next=NULL;
     }
     else{
    	 ma[count].GtPG=NULL;
    	 ma[count].GtPacc1=NULL;
    	 ma[count].acc_k=NULL;
     }
	 ma[count].acc_subK=0;/*always initialize to 0, increase to some predefined value in HERest*/
	 ma[count].acc_K=0;
     if(upPseudoLAM){
    	 ma[count].lGtPG=CreateTriMat(x,vSize);
    	 ma[count].lGtPacc1=CreateVector(x,vSize);
    	 ZeroTriMat(ma[count].lGtPG);
    	 ZeroVector(ma[count].lGtPacc1);
     }
     else{
    	 ma[count].lGtPG=NULL;
    	 ma[count].lGtPacc1=NULL;
     }
     ma[count].occ = 0.0;
     ++muC;
  }
...
static VaAcc *CreateVaAcc(MemHeap *x, int vSize, CovKind ck, int nPara, Boolean storedif, Boolean upPseudoAM, Boolean upPseudoLAM)
{
   VaAcc *va;
   int count,i;
   ...
   for(count=0;count<nPara;count++){
     ...
     case FULLC:
       ...
       break;
     default:
       HError(7170,"CreateVaAcc: bad cov kind %d",ck);break;
     }
     if(storedif&&count==nPara-1){
    	 va[count].vdif=CreateVector(x,vSize);
    	 ZeroVector(va[count].vdif);
     }
     if(upPseudoAM){
    	 va[count].hessian=(TriMat*)New(x,sizeof(TriMat)*3);
    	 va[count].gradient=(Vector*)New(x,sizeof(Vector)*3);
    	 for(i=0;i<3;i++){
    		 va[count].hessian[i]=CreateTriMat(x,vSize/3);
    		 va[count].gradient[i]=CreateVector(x,vSize/3);
    		 ZeroTriMat(va[count].hessian[i]);
    		 ZeroVector(va[count].gradient[i]);
    	 }
     }
     else{
    	 va[count].hessian=NULL;
    	 va[count].gradient=NULL;
     }
     if(upPseudoLAM){
    	 va[count].lpgradient=CreateTriMat(x,vSize);
    	 va[count].lngradient=CreateTriMat(x,vSize);
    	 ZeroTriMat(va[count].lpgradient);
    	 ZeroTriMat(va[count].lngradient);
     }
     else{
    	 va[count].lpgradient=NULL;
    	 va[count].lngradient=NULL;
     }
     va[count].occ = 0.0;
     ++vaC;
   }
...
/* CreateWtAcc: create an accumulator for mixture weights */
static RWtAcc *CreateRWtAcc(MemHeap *x, int nPara, int regwsize)
{
   RWtAcc *wa;
   int count;
   wa = (RWtAcc *) New(x,sizeof(RWtAcc)*nPara);
   for(count=0;count<nPara;count++){
	   wa[count].hjm=CreateVector(x,regwsize);
	   ZeroVector(wa[count].hjm);
	   wa[count].occ=0;
       ++rwtC;
   }
   return wa;
...
void TMAttachAccs(HMMSet *hset, MemHeap *x, int nPara,Boolean storedif)
{
   ...
   for (s=1;s<=nStreams;s++){
      ...
      for (m=1;m<=tmRec.nMix;m++){
         mp = tmRec.mixes[m];
         SetHook(mp->mean,CreateMuAcc(x,size,nPara,storedif,hset->upVTS_AM,hset->upVTS_lAM));
         SetHook(mp->cov.var,CreateVaAcc(x,size,mp->ckind,nPara,storedif,hset->upVTS_AM,hset->upVTS_lAM));
      }
   }
}
...
void AttachAccsParallel(HMMSet *hset, MemHeap *x, UPDSet uFlags, int nPara)
{
   ...
   TVWRInfo *zi;
   HLink hmm;
   int size,s,m,st,rSize;
   if(uFlags&UPFPROJ){
	   int rows=hset->fproj->targetDim;
	   int cols=hset->fproj->inputDimM;
	   hset->dAcc=(Ptr)CreateDAcc(x,rows,cols,nPara);
   }
   muC=vaC=trC=rwtC=wtC=prC=cC=0;
   NewHMMScan(hset,&hss);
   do {
      hmm = hss.hmm;
      ...
      while (GoNextState(&hss,TRUE)) {
         while (GoNextStream(&hss,TRUE)) {
            ste = hss.ste;
            ste->hook = CreateWtAcc(x,hss.M, nPara);/*WtAcc is always attached*/
            ...
            if (hss.isCont)                     /* PLAINHS or SHAREDHS */
               while (GoNextMix(&hss,TRUE)) {
        		   ...
        		   if (hss.me->rwgt&&!IsSeen(hss.me->rwgt->nUse)) {
        			   if (uFlags&UPRWGTS){
        				   switch(hset->rkind){
        				   case NONE_CTX_REG:
        				   case TEMP_CTX_REG:
        					   rSize=hset->pwidth[hss.s]*hset->temp_ctx_size;break;
        				   case SPAT_CTX_REG:
        				   case SPTP_CTX_REG:
        					   rSize=hset->regwSize;break;
        				   case HIGH_SPA_REG:
        				   case TEMP_SPA_REG:
        					   rSize=hset->confuTbl[hss.me->rwgt->spIdx][0][0];break;
        				   }
        				   hss.me->rwgt->hook=CreateRWtAcc(x,nPara,rSize);
        			   }
        			   Touch(&hss.me->rwgt->nUse);
        		   }
        		   if (!IsSeenV(hss.mp->mean)) {
        			   if ((uFlags&UPMEANS)||(uFlags&UPFPROJ))
        				   SetHook(hss.mp->mean,CreateMuAcc(x,size,nPara,hset->fMPE&&(uFlags&UPFPROJ),hset->upVTS_AM,hset->upVTS_lAM));
        			   TouchV(hss.mp->mean);
        		   }
        		   if (!IsSeenV(hss.mp->cov.var)) {
        			   if (uFlags&UPSEMIT)
        				   SetHook(hss.mp->cov.var,CreateVaAcc(x,size,FULLC,nPara,hset->fMPE&&(uFlags&UPFPROJ),hset->upVTS_AM,hset->upVTS_lAM));
        			   else if ((uFlags&UPVARS)||(uFlags&UPFPROJ))
        				   SetHook(hss.mp->cov.var,CreateVaAcc(x,size,hss.mp->ckind,nPara,hset->fMPE&&(uFlags&UPFPROJ),hset->upVTS_AM,hset->upVTS_lAM));
					   else
      ...
      if (hset->tiedTVWR&&(uFlags&UPRWGTS)){
    	  for(st=2;st<hmm->numStates;st++){
    		  zi=hmm->zvec[st].info;
    		  for(s=1;s<=hset->swidth[0];s++){
        		  for(m=1;m<=zi->tstr[s].nMix;m++){
        			  if(!IsSeen(zi->tstr[s].rwgts[m]->nUse)){
        				  switch(hset->rkind){
        				  case NONE_CTX_REG:
        				  case TEMP_CTX_REG:
        					  rSize=hset->pwidth[hss.s]*hset->temp_ctx_size;break;
        				  ...
        				  case HIGH_SPA_REG:
        				  case TEMP_SPA_REG:
        					  rSize=hset->confuTbl[zi->tstr[s].rwgts[m]->spIdx][0][0];break;
        				  }
        				  zi->tstr[s].rwgts[m]->hook=CreateRWtAcc(x,nPara,rSize);
        				  Touch(&zi->tstr[s].rwgts[m]->nUse);
        			  }
        		  }
    		  }
    	  }
   ...
   if (hset->hsKind==TIEDHS)    
      TMAttachAccs(hset, x, nPara, hset->fMPE&&(uFlags&UPFPROJ));
   if (trace&T_NAC)
      printf("AttachAccs: %d mu, %d va, %d tr, %d wt, %d pr, %d cc\n",
             muC,vaC,trC,wtC,prC,cC);
}
...
void ZeroAccsParallel(HMMSet *hset, UPDSet uFlags, int nPara)
{
   ...
   TVWRInfo *zi;
   PreComp *p;
   int i,start,end,s,m,st;
   if(nPara>0){start=0;end=nPara-1;}else{start=end=-nPara;}
   if(hset->fMPE){
	   if(uFlags&UPFPROJ){
	   	   for(i=start;i<=end;i++){
	   		   DAcc* dAcc=(DAcc*)hset->dAcc;
	   		   ZeroMatrix(dAcc[i].pGradient);
	   		   ZeroMatrix(dAcc[i].nGradient);
	   		   ZeroMatrix(dAcc[i].sqSum);
	   		   ZeroMatrix(dAcc[i].sumSq);
	   		   ZeroMatrix(dAcc[i].check1);
	   		   ZeroMatrix(dAcc[i].check2);
	   	   }
	   }
   }
   NewHMMScan(hset,&hss);
   do {
         ...
         while (GoNextStream(&hss,TRUE)) {
            ...
            if (hss.isCont)
               while (GoNextMix(&hss,TRUE)) {
        		   ...
        		   if ((uFlags&UPRWGTS) && (hss.me->rwgt&&!IsSeen(hss.me->rwgt->nUse))) {
        			   rwa = (RWtAcc *)(hss.me->rwgt->hook);
        		   	   for(i=start;i<=end;i++){
        		   		   ZeroVector(rwa[i].hjm); rwa[i].occ=0.0;
        		       }
        		   	   Touch(&hss.me->rwgt->nUse);
        		   }
      ...
      if (hset->tiedTVWR&&(uFlags&UPRWGTS)){
    	  for(st=2;st<hmm->numStates;st++){
    		  ...
    		  for(s=1;s<=hset->swidth[0];s++){
				  for(m=1;m<=zi->tstr[s].nMix;m++){
					  if(!IsSeen(zi->tstr[s].rwgts[m]->nUse)){
						  rwa = (RWtAcc *)(zi->tstr[s].rwgts[m]->hook);
						  for(i=start;i<=end;i++){
							  ZeroVector(rwa[i].hjm); rwa[i].occ=0.0;
						  }
						  Touch(&zi->tstr[s].rwgts[m]->nUse);
      ...
      if((uFlags&UPTRANS)&&(!IsSeenV(hmm->transP))) {
...
static void DumpRWtAcc(FILE *f, RWtAcc *rwa)
{
	WriteVector(f,rwa->hjm,ldBinary);
	WriteFloat(f,&(rwa->occ),1,ldBinary);
	if (!ldBinary) fprintf(f,"\n");
}
...
static void DumpMuAcc(FILE *f, MuAcc *ma)
{
   ...
   /*if(ma->GtPG){
	   for(i=0;i<3;i++){
		   WriteVector(f,ma->GtPacc1[i],ldBinary);
		   WriteTriMat(f,ma->GtPG[i],ldBinary);
	   }
   }
   else if(ma->lGtPG){
	   WriteVector(f,ma->lGtPacc1,ldBinary);
	   WriteTriMat(f,ma->lGtPG,ldBinary);
   }*/
   if(ma->acc_k){
	   NATAcc* accIter=ma->acc_k->next;
	   WriteInt(f,&(ma->acc_subK),1,ldBinary);
	   WriteInt(f,&(ma->acc_K),1,ldBinary);
	   while(accIter!=NULL){/*write a NATAcc list*/
		   WriteFloat(f,&(accIter->occ_k),1,ldBinary);
		   if(accIter->occ_k>0){
			   WriteVector(f,accIter->mu_k,ldBinary);
			   WriteVector(f,accIter->cov_k.var,ldBinary);
			   i=strlen(accIter->noise_k->id)+1;
			   WriteInt(f,&i,1,ldBinary);
			   fputs (accIter->noise_k->id, f);
			   /*WriteVector(f,ma->acc_k->noise_k->n_mean,ldBinary);
			   WriteVector(f,ma->acc_k->noise_k->h_mean,ldBinary);
			   WriteVector(f,ma->acc_k->noise_k->cov.var,ldBinary);*/
		   }
		   accIter=accIter->next;
		   ma->acc_subK--;
	   }
	   if(ma->acc_subK!=0){
		   HError(1,"Bad NATAcc writing");
	   }
   }
   else{
...
static void DumpVaAcc(FILE *f, VaAcc *va, CovKind ck)
{
	...
	/*if(va->gradient){
		for(i=0;i<3;i++){
			WriteVector(f,va->gradient[i],ldBinary);
			WriteTriMat(f,va->hessian[i],ldBinary);
		}
	}
	else if(va->lpgradient){
		WriteTriMat(f,va->lpgradient,ldBinary);
		WriteTriMat(f,va->lngradient,ldBinary);
	}*/
	if(!(va->gradient||va->lpgradient)){
		switch(ck){
			case DIAGC:
			case INVDIAGC:
			...
			case LLTC:
				...
				break;
			default:
				HError(7170,"DumpVaAcc: bad cov kind");break;
		}
	}
...
/* DumpDAcc: dump differential acc to file f */
static void DumpDAcc(FILE *f, DAcc *dAcc)
{
	WriteMatrix(f,dAcc->pGradient,ldBinary);
	WriteMatrix(f,dAcc->nGradient,ldBinary);
	WriteMatrix(f,dAcc->sqSum,ldBinary);
	WriteMatrix(f,dAcc->sumSq,ldBinary);
	WriteMatrix(f,dAcc->check1,ldBinary);
	WriteMatrix(f,dAcc->check2,ldBinary);
}
...
FILE * DumpAccsParallel(HMMSet *hset, char *fname, int n, UPDSet uFlags, int index)
{
   ...
   if(uFlags&UPFPROJ){
	   DumpDAcc(f,(DAcc*)hset->dAcc+index);
   }
   else{
	   NewHMMScan(hset, &hss);
	   do {
	      ...
	      while (GoNextState(&hss,TRUE)) {
	         while (GoNextStream(&hss,TRUE)) {
	            ...
	            if (hss.isCont){
	               while (GoNextMix(&hss,TRUE)) {
	            	  if (hss.me->rwgt&&(uFlags&UPRWGTS) && (!IsSeen(hss.me->rwgt->nUse))) {
	            		  DumpRWtAcc(f,((RWtAcc *)(hss.me->rwgt->hook))+index);
	            		  Touch(&hss.me->rwgt->nUse);
	            	  }
					  if ((uFlags&UPMEANS) && (!IsSeenV(hss.mp->mean))) {
	      ...
	      if (hset->tiedTVWR&&(uFlags&UPRWGTS)){
	    	  for(st=2;st<hmm->numStates;st++){
	    		  ...
	    		  for(s=1;s<=hset->swidth[0];s++){
		    		  for(m=1;m<=zi->tstr[s].nMix;m++){
		    			  if(!IsSeen(zi->tstr[s].rwgts[m]->nUse)){
		    				  DumpRWtAcc(f,((RWtAcc *)(zi->tstr[s].rwgts[m]->hook))+index);
		    				  Touch(&zi->tstr[s].rwgts[m]->nUse);
		    			  }
		    		  }
	    		  }
...
/* LoadWtAcc: new inc of wt acc from file f */
static void LoadRWtAcc(Source *src, RWtAcc *wa, HMMSet* hset, Boolean ml_acc, int s, int spIdx)
{
   int i,vSize;
   Vector vTemp;
   float f;
   ...
   case NONE_CTX_REG:
   case TEMP_CTX_REG:
	   vSize=hset->pwidth[s]*hset->temp_ctx_size;
	   break;
   case SPAT_CTX_REG:
   case SPTP_CTX_REG:
	   vSize=hset->regwSize;
...
static void LoadMuAcc(Source *src, MuAcc *ma, int vSize)
{
   ...
   float f;
   /*if(ma->GtPG){
	   vTemp = CreateVector(&gstack,vSize/3);
	   mTemp=CreateTriMat(&gstack,vSize/3);
	   for(i=0;i<3;i++){
		   ReadVector(src,vTemp,ldBinary);
		   ReadTriMat(src,mTemp,ldBinary);
		   for(k=1;k<=vSize/3;k++){
			   ma->GtPacc1[i][k]+=vTemp[k];
			   for(kk=1;kk<=k;kk++){
				   ma->GtPG[i][k][kk]+=mTemp[k][kk];
			   }
		   }
	   }
	   FreeVector(&gstack,vTemp);
	   ...
	   ReadTriMat(src,mTemp,ldBinary);
	   for(k=1;k<=vSize;k++){
		   ma->lGtPacc1[k]+=vTemp[k];
		   for(kk=1;kk<=k;kk++){
			   ma->lGtPG[k][kk]+=mTemp[k][kk];
		   }
	   }
	   FreeVector(&gstack,vTemp);
   }*/
   if(ma->acc_k){
	   ReadInt(src,&K1,1,ldBinary);
	   ma->acc_subK+=K1;
	   ReadInt(src,&K2,1,ldBinary);
	   ma->acc_K+=K2;
	   for(k=0;k<K1;k++){
		   NATAcc *acc_k=(NATAcc*)New(&gstack,sizeof(NATAcc));
		   ReadFloat(src,&f,1,ldBinary);
		   acc_k->occ_k=f;
		   if(f>0){
			   acc_k->mu_k=CreateVector(&gstack,vSize);
			   acc_k->cov_k.var=CreateVector(&gstack,vSize);
			   ReadVector(src,acc_k->mu_k,ldBinary);
			   ReadVector(src,acc_k->cov_k.var,ldBinary);
			   acc_k->noise_k=(NoiseModel*)New(&gstack,sizeof(NoiseModel));
			   ReadInt(src,&i,1,ldBinary);
			   fgets(acc_k->noise_k->id,i,src->f);
			   /*acc_k->noise_k->n_mean=CreateVector(&gstack,vSize);
			   acc_k->noise_k->h_mean=CreateVector(&gstack,vSize);
			   acc_k->noise_k->cov.var=CreateVector(&gstack,vSize);
			   ReadVector(src,acc_k->noise_k->n_mean,ldBinary);
			   ReadVector(src,acc_k->noise_k->h_mean,ldBinary);
			   ReadVector(src,acc_k->noise_k->cov.var,ldBinary);*/
			   NATAcc *temp=ma->acc_k->next;/*store a list of examples*/
			   ma->acc_k->next=acc_k;
			   acc_k->next=temp;
		   }
	   }
...
static void LoadVaAcc(Source *src, VaAcc *va, int vSize, CovKind ck)
{
	...
	/*if(va->gradient){
		...
		for(i=0;i<3;i++){
			...
			ReadTriMat(src,mTemp,ldBinary);
			for(k=1;k<=vSize/3;k++){
				va->gradient[i][k]+=vTemp[k];
				for(kk=1;kk<=k;kk++){
					va->hessian[i][k][kk]+=mTemp[k][kk];
				}
			}
		}
		FreeVector(&gstack, vTemp);
		...
		for(k=1;k<=vSize;k++){
			for(kk=1;kk<=k;kk++){
				va->lpgradient[k][kk]+=mTemp[k][kk];
			}
		}
		ReadTriMat(src,mTemp,ldBinary);
		for(k=1;k<=vSize;k++){
			for(kk=1;kk<=k;kk++){
				va->lngradient[k][kk]+=mTemp[k][kk];
			}
		}
...
/* LoadDAcc: load differential acc to file f */
static void LoadDAcc(Source *src, DAcc *dAcc,int rows, int cols)
{
	int i,j;
	Matrix gTemp=CreateMatrix(&gstack,rows,cols);
	Matrix cTemp=CreateMatrix(&gstack,rows,2);
	ReadMatrix(src,gTemp,ldBinary);
	for(i=1;i<=rows;i++){
		for(j=1;j<=cols;j++){
			dAcc->pGradient[i][j]+=gTemp[i][j];
		}
	}
	ReadMatrix(src,gTemp,ldBinary);
	for(i=1;i<=rows;i++){
		for(j=1;j<=cols;j++){
			dAcc->nGradient[i][j]+=gTemp[i][j];
		}
	}
	ReadMatrix(src,gTemp,ldBinary);
	for(i=1;i<=rows;i++){
		for(j=1;j<=cols;j++){
			dAcc->sqSum[i][j]+=gTemp[i][j];
		}
	}
	ReadMatrix(src,gTemp,ldBinary);
	for(i=1;i<=rows;i++){
		for(j=1;j<=cols;j++){
			dAcc->sumSq[i][j]+=gTemp[i][j];
		}
	}
	ReadMatrix(src,cTemp,ldBinary);
	for(i=1;i<=rows;i++){
		for(j=1;j<=2;j++){
			dAcc->check1[i][j]+=cTemp[i][j];
		}
	}
	ReadMatrix(src,cTemp,ldBinary);
	for(i=1;i<=rows;i++){
		for(j=1;j<=2;j++){
			dAcc->check2[i][j]+=cTemp[i][j];
		}
	}
	FreeMatrix(&gstack,gTemp);
}
...
Source LoadAccsParallel(HMMSet *hset, char *fname, UPDSet uFlags, int index)
{
   ...
   if(uFlags&UPFPROJ){
	   int rows=hset->fproj->targetDim;
	   int cols=hset->fproj->inputDimM;
   	   LoadDAcc(&src,(DAcc*)hset->dAcc+index,rows, cols);
   }
   else{
	   NewHMMScan(hset, &hss);
	   do {
	      ...
	      while (GoNextState(&hss,TRUE)) {
	         while (GoNextStream(&hss,TRUE)) {
	            ...
	            if (hss.isCont){
	               while (GoNextMix(&hss,TRUE)) {
	            	  if (hss.me->rwgt&&(uFlags&UPRWGTS) && (!IsSeen(hss.me->rwgt->nUse))) {
	            		  LoadRWtAcc(&src,((RWtAcc *)(hss.me->rwgt->hook))+index,hset,index==0, hss.s, hss.me->rwgt->spIdx);
	            		  Touch(&hss.me->rwgt->nUse);
	            	  }
					  if ((uFlags&UPMEANS) && (!IsSeenV(hss.mp->mean))) {
	      ...
	      if (hset->tiedTVWR&&(uFlags&UPRWGTS)){
	    	  for(st=2;st<hmm->numStates;st++){
	    		  ...
	    		  for(s=1;s<=hset->swidth[0];s++){
	    			  for(m=1;m<=zi->tstr[s].nMix;m++){
	    				  if(!IsSeen(zi->tstr[s].rwgts[m]->nUse)){
	    					  LoadRWtAcc(&src,((RWtAcc *)(zi->tstr[s].rwgts[m]->hook))+index,hset,index==0,s,zi->tstr[s].rwgts[m]->spIdx);
	    					  Touch(&zi->tstr[s].rwgts[m]->nUse);
	    				  }
	    			  }
	    		  }
...
void RestorePDF(MixPDF *mp, int index, HMMSet *hset){
   int i,j;
   ...
   case FULLC: case LLTC:
      ...
      break;
   default: HError(7191, "Unknown ckind [RestoreAccsParallel]");break;
   }
}
...
void RestoreAccsParallel(HMMSet *hset, int index)
{
   ...
   if(hset->hsKind==TIEDHS){
      for (s=1; s<=hset->swidth[0]; s++){
         size = hset->swidth[s];
         for (m=1;m<=hset->tmRecs[s].nMix; m++)
            RestorePDF(hset->tmRecs[s].mixes[m], index, hset);
      }
   } else {
      NewHMMScan(hset, &hss);
      while (GoNextMix(&hss,FALSE)) {
         RestorePDF(hss.mp, index, hset);
      }
      EndHMMScan(&hss);
   }
}
   ...
   while(GoNextState(&hss,FALSE)){ /*skip over hmm boundaries.*/
      while(GoNextStream(&hss,TRUE)){ /*Don't skip over state boundaries.*/
         ...
         case TIEDHS:
            ...
            break;
         default: HError(1, "ScaleAccs- unknown hsKind.");break;
         }
      }
   }
   EndHMMScan(&hss);
