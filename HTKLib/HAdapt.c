...
static Boolean saveSemiTiedMean = TRUE;
...
void InitAdapt (XFInfo *xfinfo) 
{
   ...
   if (nParm>0){
	 ...
	 xformBlockSize = ParseConfIntVec(&infoStack,buf);
      ...
      if (GetConfBool(cParm,nParm,"SAVESEMITIEDMEAN",&b)) saveSemiTiedMean = b;
...
/* 
   ...
   storage as required
*/
static void UpdateMInfo(HMMSet *hset, AdaptXForm *xform, Boolean force)
{
   Boolean adaptMean, adaptCov;
   AdaptXForm *xf;
   int size;
         ...
         while (GoNextStream(&hss,TRUE)) {            
            if (hss.isCont)                     /* PLAINHS or SHAREDHS */
               while (GoNextMix(&hss,TRUE)) {
                  mp = hss.mp;
                  mi = GetMInfo(mp);
                  if ((adaptMean && (mi->mean == NULL))||force) {
                	  size = VectorSize(mp->mean);
                	  if(mi->mean==NULL)
...
void UpdateBaseAccs(Vector svec)
{
   ...
   for (b=1;b<=bclass->numClasses;b++) {
      ...
      if ((ra->bTriMat != NULL) && (ra->bVector[1]>0)) {    
         acc = ra->bVector;/*scale factor, Lr/covar[i]*/
         nblock = (int)(ra->bDiagMat[0]);/*outer product*/
         ...
         for (bl=1, cnt=1; bl<=nblock;bl++){
            ...
            m = ra->bDiagMat[bl];
            for (i=1, cnti=cnt; i<=bsize; i++,cnti++) { /* Fill the outer product for current frame */
    ...
    for (bl=1,cnti=1;bl<=nblock;bl++) {
      ...
      for (i=1;i<=bsize;i++,cnti++) { /* Fill the accumulate stores */
        ...
        for (j=1; j<=bsize; j++)
          for (k=1; k<=j; k++)
            tm[j][k] += m[j][k] * acc[cnti];/*\sum_t \sum_m (occ_m/var_m)*O_t^2 */
      }
         }
...
static void AccBaseTriMat(HMMSet *hset, double Lr, Vector svec, MixPDF *mp, int t)
{
   ...
   if (t != baseTriMatTime) {
     ...
     if (headac == NULL ) 
       UpdateBaseAccs(svec);/*as we only need once outer product of the observation, after that, it is a simple multiplication of the outer product*/
     else  {
   ...
   if  (headac == NULL) {    
     for (i=1;i<=vsize;i++) {
       if (mp->ckind==INVDIAGC)
	 regAcc->bVector[i] += covar[i]*Lr;/*now we accumulate the scale factor*/
       else
...
static void Tri2DMatx (DMatrix m1, DMatrix m2)
{
  int i,j,nrows,ncols;
...
void ApplyXForm2Vector(LinXForm *linXForm, Vector mean)
{  
   Vector vec, bias;
   int size,b,bsize;
   Matrix A;
...
static void CompXForm(MixPDF *mp, AdaptXForm *xform, AInfo *ai)
{
  ...
  numXf = xform->xformWgts.assign[ai->baseClass];
  /* Apply linear transformations to the parameters */
  if (numXf > 0) { /* Allows support when no transforms have been generated, CMLLR is always applied as feature transformation for efficient computing*/
    xformSet = xform->xformSet;
    switch (xformSet->xkind) {
    case MLLRMEAN:
...
static void UpdateSemiTiedXForm(AccStruct *accs, LinXForm *xf, LinXForm *ixf)
{
   ...
   for (b=1,cnt=1;b<=IntVecSize(accs->blockSize);b++) {
     ...
     if (ixf == NULL) {
        ...
        for (i=1,cnti=cnt;i<=bsize;i++, cnti++) {
           InvG[cnti] = CreateDMatrix(&gstack,dim,dim);
           DTri2DMat(accs->G[cnti],InvG[cnti]);/*BUGGGGGGGGGGGGGGGGGGGGGGGGGGGGG?*/
           InvSVD(InvG[cnti], u, w, v, InvG[cnti]);
           for (j=1;j<=bsize;j++)
...
static Boolean GenClassXForm(BaseClass *bclass, AdaptXForm *xform, AccStruct** vtsAccs)
{
  AccStruct *accs;
  int b;
  ILink i;
...
void AccAdaptFrame(HMMSet *hset, double Lr, Vector svec, MixPDF *mp, int t)
{
   ...
   if (ra->spSum != NULL)
      for (i=1;i<=vsize;i++)
         ra->spSum[i] += Lr*svec[i];/*first order accumulator*/
   if (ra->spSumSq != NULL)
      for (i=1;i<=vsize;i++)
         ra->spSumSq[i] += Lr*svec[i]*svec[i];/*second order accumulator*/
   if (ra->bTriMat != NULL) 
...
void SetXForm(HMMSet *hset, AdaptXForm *xform, Boolean force)
{
   ...
   if (storeMInfo) {
      if (hset->attMInfo) {
         if (force||!CompareMInfo(hset,xform))
            UpdateMInfo(hset,xform,force);
      } else {
         SetMInfo(hset,xform);
      }
   }
...
Boolean GenAdaptXForm(HMMSet *hset, AdaptXForm* xform, AccStruct** vtsAccs)
{
   AdaptKind akind;
   ...
   case TREE: 
      ...
      break;
   case BASE:
      genXForm = GenClassXForm(xform->bclass,xform,vtsAccs);
      break;
   default:
      HError(999,"Only TREE and BASE adaptation kinds currently supported");break;
   }
...
void SpkrVTSAccInit(HMMSet *hset, XFInfo *xfInfo, char* datafn){
	static char newMn[MAXSTRLEN];
	char spkr[MAXSTRLEN];
	...
	BaseClass* bclass=xfInfo->outXForm->bclass;
	int b;
	AccStruct* accs=NULL;
	xfInfo->VTSAccs=(AccStruct**)New(hset->hmem,sizeof(AccStruct*)*(bclass->numClasses+1));
	for (b=1;b<=bclass->numClasses;b++) {
		/* Accumulate structure regenerated each time as this will handle
		       streams of different sizes simply */
		if (strmProj)
			accs= CreateAccStruct(hset->hmem,xform,xform->hset->vecSize,GetBlockSize(xform,b));
		else
			accs = CreateAccStruct(hset->hmem,xform,GetVecSizeClass(bclass,b), GetBlockSize(xform,b));
		xfInfo->VTSAccs[b]=accs;
	}
}

/*remember to reset the accumulator for each new utterance*/
void SpkrVTSAccStat(XFInfo *xfInfo){
	AdaptXForm* xform=xfInfo->outXForm;
	BaseClass* bclass=xfInfo->outXForm->bclass;
	int b;
	ILink i;
   ...
   if (xfinfo->useOutXForm) { /* if there is an output transform to be generated */
      ...
      if ((datafn == NULL) || ((coutspkr!=NULL) && strcmp(spkr,coutspkr))) {
         ...
         if (nspkr>0) { /* nothing to generate if first speaker */
            ...
            MakeFN(coutspkr,NULL,xfinfo->outXFormExt,newMn);
            if(xfinfo->VTSAccs==NULL){/*in case of VTS, there is another function to create the transform, SpkrVTSAccInit*/
            ...
            GenAdaptXForm(hset,xfinfo->outXForm,xfinfo->VTSAccs);
	    if (mllrDiagCov) xfinfo->outXForm = diagCovXForm;
            ...
            if (saveSpkrModels) { 
               ...
               */
               SetXForm(hset,xfinfo->outXForm,xfinfo->hasVTS);
		 ...
		 SetXForm(hset,NULL,xfinfo->hasVTS);
	       }
            }
            spkrChange = TRUE;
   ...
   if (xfinfo->useInXForm  && (datafn != NULL)) {
      ...
      if (((cinspkr!=NULL) && strcmp(spkr,cinspkr)) || (resetHMMSet)) {
         ...
         if (xfinfo->al_hset != NULL) {
            ...
            SetXForm(xfinfo->al_hset,xfinfo->al_inXForm,xfinfo->hasVTS);
...
void lVtsAdapt(HMMSet* hset, Vector lmu_n, Vector lmu_h, TriMat lcv_n, Matrix lmus_x, Matrix* lcvs_x, DMatrix* lGs){
	HMMScanState hss;
	int numFrms, lnumCeps,i,j;
	DMatrix dct,idct;
	lnumCeps=hset->vecSize;/*e.g. 13*8 or 13*9 */
	numFrms=lnumCeps/numCepCoef;
	DVector lx_cep_mu, ly_cep_mu, ln_cep_mu, lh_cep_mu;
	DMatrix lx_cep_cv, ly_cep_cv, ln_cep_cv;
	DMatrix lG,lF,lGt,lFt,GCx,GCxGt,FCn,FCnFt;
	DVector lexpanPoint;/*mu_n-mu_x-mu_h*/
	DVector lg0;
	TriMat tempTri;
	MixPDF* mp;
	lx_cep_mu=CreateDVector(&gstack,lnumCeps);
	ln_cep_mu=CreateDVector(&gstack,lnumCeps);
	lh_cep_mu=CreateDVector(&gstack,lnumCeps);
	ly_cep_mu=CreateDVector(&gstack,lnumCeps);
	lexpanPoint=CreateDVector(&gstack,lnumCeps);
	lg0=CreateDVector(&gstack,lnumCeps);
	lx_cep_cv=CreateDMatrix(&gstack,lnumCeps, lnumCeps);
	ln_cep_cv=CreateDMatrix(&gstack,lnumCeps, lnumCeps);
	ly_cep_cv=CreateDMatrix(&gstack,lnumCeps, lnumCeps);
	tempTri=CreateTriMat(&gstack,lnumCeps);
	dct=lLiftDCT(&gstack, numFrms, numChans, numCepCoef,cepLifter);
	idct=ilLiftDCT(&gstack, numFrms,numChans,numCepCoef,cepLifter);
	lG=CreateDMatrix(&gstack,lnumCeps,lnumCeps);
	lF=CreateDMatrix(&gstack,lnumCeps,lnumCeps);
	lGt=CreateDMatrix(&gstack,lnumCeps,lnumCeps);
	lFt=CreateDMatrix(&gstack,lnumCeps,lnumCeps);
	...
	for(i=1;i<=lnumCeps;i++){
		...
		lh_cep_mu[i]=lmu_h[i];
		for(j=1;j<=i;j++){
			ln_cep_cv[i][j]=lcv_n[i][j];
			ln_cep_cv[j][i]=lcv_n[i][j];
		}
	}
	...
	while(GoNextMix(&hss,FALSE)){
		...
		Vector mean=lmus_x?lmus_x[mixIdx]:mp->mean;
		Vec2DVec(mean, lx_cep_mu);
		if(lcvs_x)
			Mat2DMat(lcvs_x[mixIdx], lx_cep_cv);
		else
			DCovInvert(mp->cov.inv,lx_cep_cv);
		/**************************************Preparation*****************************************/
		for(i=1;i<=lnumCeps;i++){/*mu=mu_n-mu_x-mu_h*/
			lexpanPoint[i]=ln_cep_mu[i]-lx_cep_mu[i]-lh_cep_mu[i];
		}
		VTS(&gstack, dct, idct, lexpanPoint, lG, lGt, lF, lFt, lg0);
		if(lGs){
			CopyDMatrix(lG,lGs[mixIdx]);
		}
		/**************************************static parameters***************************************/
		for(i=1;i<=lnumCeps;i++){
			ly_cep_mu[i]=lx_cep_mu[i]+lh_cep_mu[i]+lg0[i];/*mu_y=mu_x+mu_h+g(mu_n-mu_x-mu_h)*/
		}
		VtsUpVar(lG, lGt, lF, lFt, GCx, GCxGt, FCn, FCnFt, lx_cep_cv, ln_cep_cv, ly_cep_cv);
		DVec2Vec(ly_cep_mu,mp->mean);
		DMat2Tri(ly_cep_cv,tempTri);
		CovInvert(&gstack, tempTri,mp->cov.inv);
		FixFullGConst(mp,-CovDet(&gstack, mp->cov.inv));
	}
	EndHMMScan(&hss);
	FreeDVector(&gstack,lx_cep_mu);
}

/**
 * regular VTS compensation
 */
void VtsAdapt(HMMSet* hset, Vector mu_n, Vector mu_h, Vector cv_n, Matrix mus_x, Matrix cvs_x){
	HMMScanState hss;
	int i,j;
	DMatrix dct,idct;
	...
	DVector n_cep_mu, h_cep_mu;
	DVector y_cep_mu, dy_cep_mu, ddy_cep_mu;
	DVector x_cep_cv, dx_cep_cv, ddx_cep_cv;
	DVector y_cep_cv, dy_cep_cv, ddy_cep_cv;
	DVector n_cep_cv, dn_cep_cv, ddn_cep_cv;
	DMatrix G,Gt,F,Ft;
	DVector exPoint;/*mu_n-mu_x-mu_h*/
	DVector g0;
	MixPDF* mp;
	x_cep_mu=CreateDVector(&gstack,numCepCoef);dx_cep_mu=CreateDVector(&gstack,numCepCoef);ddx_cep_mu=CreateDVector(&gstack,numCepCoef);
	y_cep_mu=CreateDVector(&gstack,numCepCoef);dy_cep_mu=CreateDVector(&gstack,numCepCoef);ddy_cep_mu=CreateDVector(&gstack,numCepCoef);
	exPoint=CreateDVector(&gstack,numCepCoef);
	g0=CreateDVector(&gstack,numChans);
	x_cep_cv=CreateDVector(&gstack, numCepCoef);dx_cep_cv=CreateDVector(&gstack, numCepCoef);ddx_cep_cv=CreateDVector(&gstack, numCepCoef);
	y_cep_cv=CreateDVector(&gstack, numCepCoef);dy_cep_cv=CreateDVector(&gstack, numCepCoef);ddy_cep_cv=CreateDVector(&gstack, numCepCoef);
	dct=liftDCT(&gstack, numChans, numCepCoef, cepLifter);idct=iLiftDCT(&gstack, numChans,numCepCoef, cepLifter);
	G=CreateDMatrix(&gstack,numCepCoef,numCepCoef);F=CreateDMatrix(&gstack,numCepCoef,numCepCoef);
	Gt=CreateDMatrix(&gstack,numCepCoef,numCepCoef);Ft=CreateDMatrix(&gstack,numCepCoef,numCepCoef);
	n_cep_mu=CreateDVector(&gstack,numCepCoef);h_cep_mu=CreateDVector(&gstack,numCepCoef);
	n_cep_cv=CreateDVector(&gstack, numCepCoef);dn_cep_cv=CreateDVector(&gstack, numCepCoef);ddn_cep_cv=CreateDVector(&gstack, numCepCoef);
	/**********************Load noise model********************************/
	for(i=1;i<=numCepCoef;i++){
		n_cep_mu[i]=mu_n[i];
		h_cep_mu[i]=mu_h[i];
		n_cep_cv[i]=cv_n[i];
		dn_cep_cv[i]=cv_n[numCepCoef+i];
		ddn_cep_cv[i]=cv_n[numCepCoef*2+i];
	}
	...
	while(GoNextMix(&hss,FALSE)){
		...
		mixIdx=mp->mIdx;
		if(hss.me->rwgt){
			hss.me->rwgt->cached_t=-1;/*important*/
		}
		if(mixIdx<=0)continue;
		Vector mean=NULL;
		Vector var=NULL;
		/*******************************load clean acoustic model*********************************/
		if(cvs_x){
			mean=mus_x[mixIdx];var=cvs_x[mixIdx];
		}
		else{
			if(mp->ckind==INVDIAGC){/*we want the variance rather than its inverse*/
				for(i=1;i<=vSize;i++){
					mp->cov.var[i]=1/mp->cov.var[i];/*its ok to reverse the variance, as it will be updated eventually*/
				}
			}
			mean=mp->mean;
			var=mp->cov.var;
		...
		for(i=1;i<=numCepCoef;i++){
			x_cep_mu[i]=mean[i];dx_cep_mu[i]=mean[numCepCoef+i];ddx_cep_mu[i]=mean[numCepCoef*2+i];
			x_cep_cv[i]=var[i];dx_cep_cv[i]=var[numCepCoef+i];ddx_cep_cv[i]=var[numCepCoef*2+i];
		}
		/********************************Jacobian calculation************************************/
		for(i=1;i<=numCepCoef;i++){/*mu=mu_n-mu_x-mu_h*/
			exPoint[i]=n_cep_mu[i]-x_cep_mu[i]-h_cep_mu[i];
		}
		VTS(&gstack, dct, idct, exPoint, G, Gt, F, Ft, g0);
		/*******************************Compensation****************************************/
		for(i=1;i<=numCepCoef;i++){
			y_cep_mu[i]=x_cep_mu[i]+h_cep_mu[i]+g0[i];/*mu_y=mu_x+mu_h+g(mu_n-mu_x-mu_h)*/
			DDotProduct(G[i],dx_cep_mu,&dy_cep_mu[i]);/*dy_mu=A*dx_mu+(I-A)*dn_mu*/
			DDotProduct(G[i],ddx_cep_mu,&ddy_cep_mu[i]);/*ddy_mu=A*ddx_mu+(I-A)*ddn_mu*/
		}
		dVtsUpVar(G, Gt, F, Ft, x_cep_cv, n_cep_cv, y_cep_cv);
		dVtsUpVar(G, Gt, F, Ft, dx_cep_cv, dn_cep_cv, dy_cep_cv);
		dVtsUpVar(G, Gt, F, Ft, ddx_cep_cv, ddn_cep_cv, ddy_cep_cv);
		/****************************update the back-end acoustic model**************************/
		for(i=1;i<=numCepCoef;i++){
			mp->mean[i]=y_cep_mu[i];mp->mean[numCepCoef+i]=dy_cep_mu[i];mp->mean[numCepCoef*2+i]=ddy_cep_mu[i];
			mp->cov.var[i]=(mp->ckind==DIAGC)?y_cep_cv[i]:(1/y_cep_cv[i]);
			mp->cov.var[numCepCoef+i]=(mp->ckind==DIAGC)?dy_cep_cv[i]:(1/dy_cep_cv[i]);
			mp->cov.var[numCepCoef*2+i]=(mp->ckind==DIAGC)?ddy_cep_cv[i]:(1/ddy_cep_cv[i]);
		}
		if(mp->ckind==DIAGC){
			FixDiagGConst(mp);
		}
		else{/*either DIAGC or INVDIAGC*/
			FixInvDiagGConst(mp);
		}
	}
	EndHMMScan(&hss);
