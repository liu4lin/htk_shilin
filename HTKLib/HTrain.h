...
enum _UPDSet{UPMEANS=1,UPVARS=2,UPTRANS=4,UPMIXES=8,UPXFORM=16,UPMAP=32,UPSEMIT=64,UPFPROJ=128,UPRWGTS=256};
...
typedef struct {     /* attached to StreamElem */
	Vector hjm;		 /* [1..N], dynamic weight parameter size, for denominator case, this acc were collected after multiplication with static weight, which is good for tied system, as it is optimized given the previous static weights*/
	float occ;
} RWtAcc;

typedef struct _NATAcc {/* an example of utterance containing high occ for this component, used for checking auxiliary function change during NAT acoustic model update*/
	float occ_k;
	Vector mu_k;
	Covariance cov_k;
	NoiseModel* noise_k;
...
typedef struct {     /* attached to mean vector */
   ...
   Vector udif;		 /* indirect differential */
   Vector* GtPacc1;	 /* NAT HMM acoustic model mean acc*/
   TriMat* GtPG;	 /* NAT HMM acoustic model mean square acc*/
   Vector lGtPacc1;	 /* NAT long span acoustic model mean acc*/
   TriMat lGtPG;	 /* NAT long span acoustic model square acc*/
   float occ;        /* occ for states sharing this mpdf */
   NATAcc* acc_k;	 /* an example list, this is a header*/
   int acc_subK;	 /* subset of examples for each batch job, not the overall examples, to access the overall examples, use the list*/
   int acc_K;		 /* fullset examples*/
} MuAcc;

typedef struct {     /* attached to covariance vector/matrix */
   Covariance cov;   /* covariance counts */
   Vector vdif;		 /* indirect differential */
   Vector* gradient;/* NAT acoustic model variance acc*/
   TriMat* hessian; /* NAT acoustic model variance square acc*/
   TriMat lpgradient;/* NAT long span acoustic model variance acc*/
   TriMat lngradient;
   float occ;        /* occ for states sharing this mpdf */
} VaAcc;

typedef struct {	/* the statistics for differential calculation, define a global variable or attached to HMMSet */
	Matrix pGradient;		/*[1...ncls][][], first-order. positive contribution*/
	Matrix nGradient;		/*[1...ncls][][], first-order, negative contribution, always store the positive value*/
	Matrix sumSq;
	Matrix sqSum;
	Matrix check1;
	Matrix check2;
