...
static void AttachWtTrAccs(HMMSet *hset, MemHeap *x)
{
   ...
   do {
      ...
      if (!IsSeenV(hmm->transP)) {
         SetHook(hmm->transP, CreateTrAcc(x,hmm->numStates)); /* why use such method to hook the attached data? */
         TouchV(hmm->transP);       
      }
                    ...
                    DVector aqt, DVector bqt, DVector bq1t, LogDouble pr)
{
   ...
   for (i=1;i<=N;i++) {
      ...
      x -= pr;/*log{alpha(t, q, j=i)*beta(t, q, j=i)/alpha(t=T,q=N)}=aqt[i]+bqt[i]-pr */
      occt[i] = (x>MINEARG) ? exp(x) : 0.0;/* gamma(t, q, j=i) */
   }
   if (trace&T_OCC) {
      occaq = occa[q];
...
static int CreateInsts(FBInfo *fbInfo, AlphaBeta *ab, int Q, Transcription *tr)
{
   ...
   al_hset=fbInfo->al_hset; /* HMMs to use for alignment */
   up_hset=fbInfo->up_hset; /* set of HMMs to be re-estimated */
   ...
   qt=0;
   for (lab=tr->head->head->succ,q=1; lab->succ!= NULL; lab=lab->succ,q++){/* handle one lab in one iteration */
      /* align models */
      if((macroName=FindMacroName(al_hset,'l',lab->labid))==NULL)/* search the model from HMM set by labid */
      ...
      qIds[q] = macroName->id;
      ta = (TrAcc *)GetHook(al_qList[q]->transP);/* the sufficient statistics is accumulated for each HMM or each state or each Gaussian component, therefore one instance of HMM is enough for each lab */
      qt += (qDms[q] = ta->minDur);/* a HMM with N states has a minimum duration, i.e. N-2 */
                      ...
                      int Q, int skipstart, int skipend)
{
   ...
   /* forward implementation @htkbook.pp144 when t=1*/
   p = ab->pInfo;
   eq = p->qHi[1];
   for (q=1; q<=eq; q++){
      hmm = ab->al_qList[q]; Nq = hmm->numStates;
      aq = ab->alphat[q];
      aq[1] = (q==1)?0.0:ab->alphat[q-1][1]+a1N;/* alpha(t=1, q, j=1) for q?=1*/
      if((outprob = ab->otprob[1][q]) == NULL)
         HError(7322,"InitAlpha: Outprob NULL in model %d in InitAlpha",q);
      for (j=2;j<Nq;j++) {/* alpha(t=1, q, j) for j=2...Nq-1 */
         a = hmm->transP[1][j];
                      ...
                      int Q, int T, LogDouble pr, int skipstart, int skipend)
{
   ...
   alphat  = ab->alphat;/* array[1..Q][1..Nq] of prob */
   alphat1 = ab->alphat1;/* alpha[t-1] */
...
static float * ShStrP(HMMSet *hset, StreamElem *ste, Vector v, int t,
		       AdaptXForm *xform, MemHeap *abmem, HDFInfo *hdfInfo, int s)
{
	WtAcc *wa;
	MixtureElem *me;
	MixPDF *mp;
	...
	wa = (WtAcc *)ste->hook;
	if (wa->time==t&&!hset->tiedTVWR)           /* seen this state before, note that since tied tvwr introduced a separate state tying such that we cant rely on this cache anymore */
		outprobjs = wa->prob;
	else {
		if(hset->fMPE){
			v=ApplyFprojection(hdfInfo,t);
		}
		if(hset->TVWR){
			ApplyWprojection(ste, hdfInfo, t, s);
		}
		M = ste->nMix;/* num mixtures in this stream */
		outprobjs = NewOtprobVec(abmem,M);/* create prob vector size [0..M] */
		me = ste->spdf.cpdf+1;
		/*do pre-computing for time varying weights*/
		if (M==1){                 /* Single Mix Case */
			...
			for (m=1;m<=M;m++,me++) {
				...
				if (swt>LMINMIX){
					...
					mixp = MOutP(ApplyCompFXForm(mp,v,xform,&det,t),mp);
					mixp += det;
					mixp +=(hdfInfo&&hdfInfo->logKt?hdfInfo->logKt[t-hdfInfo->StartTime]:0);
					x = LAdd(x,wt+mixp);
					outprobjs[m] = mixp;
				}
			}
                      ...
                      Observation ot, int t, int S, int qHi, int qLo)
{
   ...
   p = ab->pInfo;
   otprob = ab->otprob;/* array[1..T][1..Q][2..Nq-1][0..S][0..M] of prob */
      ...
      if (otprob[t][q] == NULL)
         {
            ...
            for (j=2;j<Nq;j++){
               ...
               for (s=1;s<=S;s++,ste++){
                  ...
                  case SHAREDHS: 
                	  if (S==1){
                		  outprobj[0] = ShStrP(hset,ste,ot.fv[s],t,fbInfo->al_inXForm,&ab->abMem, &fbInfo->hdfInfo, 1);/* stream Outp calculation exploiting sharing */
                	  }
                	  else {
                		  if (((WtAcc *)ste->hook)->time==t)
                			  seenState=TRUE;
                		  else
                			  seenState=FALSE;
                		  outprobj[s] = ShStrP(hset,ste,ot.fv[s],t,fbInfo->al_inXForm,&ab->abMem, &fbInfo->hdfInfo, s);
                	  }
                	  break;
                  default:
                	  if (S==1)
...
static void SetBeamTaper(PruneInfo *p, short *qDms, int Q, int T)
{
   ...
   /* Set leading taper */
   q=1;dq=qDms[q];i=0;/* qDms: array[1..Q] of minimum model duration */
   for (t=1;t<=T;t++) {
      while (i==dq) {
         i=0;
         if (q<Q) q++,dq=qDms[q];
      ...
      p->qHi[t]=q;/*111, 222, 333, ..., QQQ, QQ...Q*/
      i++;
   }
   q=Q;dq=qDms[q];i=0;
   for (t=T;t>=1;t--) {
...
static LogDouble SetBeta(AlphaBeta *ab, FBInfo *fbInfo, UttInfo *utt)
{
   ...
   skipend = fbInfo->skipend;
   pbuf=utt->pbuf;
   S=utt->S;/* number of data streams */
   Q=utt->Q;/* number of models in transcription */
   T=utt->T;
   p=ab->pInfo;
   beta=ab->beta;

   maxP = CreateDVector(&gstack, Q);   /* for calculating beam width */
   /* backward implementation @htkbook.pp144 */
   /* Last Column t = T */
   p->qHi[T] = Q; endq = p->qLo[T];
   Setotprob(ab,fbInfo,pbuf,utt->ot,T,S,Q,endq);/* allocate and calculate otprob matrix at time t, emitting prob */
   ...
   for (q=Q; q>=endq; q--){
      ...
      bqt[Nq] = (q==Q)?0.0:beta[T][q+1][lNq]+a1N;/* beta(T, q, Nq) */
      for (i=2;i<Nq;i++) 
         bqt[i] = hmm->transP[i][Nq]+bqt[Nq];/* beta(T, q, i) */
      outprob = ab->otprob[T][q];/* b(T, q, j) j=2...Nq-1 */
      x = LZERO;/* to calculate beta(T, q, 1) */
      for (j=2; j<Nq; j++){
      ...
      bqt[1] = x;
      lNq = Nq; a1N = hmm->transP[1][Nq];/*lNq: last Nq, i.e. Nq+1 since q is decreasing from Q*/
      if (x>gMax) {
         gMax = x; q_at_gMax = q;
      }
   }
   ...
   for (t=T-1;t>=1;t--) {      
      ...
      gMax = LZERO;   q_at_gMax = 0;    /* max value of beta at time t */
      startq = p->qHi[t+1]; /* the HMM arc with max possible index q */
      endq = (p->qLo[t+1]==1)?1:((p->qLo[t]>=p->qLo[t+1])?p->qLo[t]:p->qLo[t+1]-1);/* the HMM arc with min possible index q*/
      ...
      for (q=startq;q>=endq;q--) {/* scan from arc with higher q */
         lMax = LZERO;                 /* max value of beta in model q */
         hmm = ab->al_qList[q]; 
         Nq = hmm->numStates;
         bqt = beta[t][q] = NewBetaVec(&ab->abMem,Nq);
         bqt1 = beta[t+1][q];/* bqt1: beta(t, q, t+1) */
         bq1t1 = (q==Q)?NULL:beta[t+1][q+1];/*bq1t1: beta(t, q+1, t+1) */
         outprob = ab->otprob[t+1][q];
         bqt[Nq] = (bq1t1==NULL)?LZERO:bq1t1[1];/* beta(t, q, Nq) for q?=Q */
         if (q<startq && a1N>LSMALL)
            bqt[Nq]=LAdd(bqt[Nq],beta[t][q+1][lNq]+a1N);/* if q!=Q */
         for (i=Nq-1;i>1;i--){
   ...
   do
      {
         ...
         qt=CreateInsts(fbInfo,ab,utt->Q,utt->tr);/* create array of hmm instances for current transcription */
         if (qt>utt->T) {/* qt:the minimum number of frames needed for this particular transcription */
            if (trace&T_TOP)
         ...
         CreateOtprob(ab,utt->T);/* array[1..T][1..Q][2..Nq-1][0..S][0..M] of prob */
         lbeta=SetBeta(ab,fbInfo,utt);/* core procedure for backward calculation */
      ...
      for (j=2;j<=N;j++) {
         if (i==1 && j<N) {                  /* entry transition */
            ...
            if (i>1 && j<N && bqt1!=NULL) {     /* internal transition */
               x = aqt[i]+ai[j]+outprob1[j][0][0]+bqt1[j]-pr;/* R(t,i,j)=P(state(t-1)=i,state(t)=j|O(1,T)) */
               if (x>MINEARG) ti[j] += exp(x);
            } else
...
static float UpMixParms(FBInfo *fbInfo, int q, HLink hmm, HLink al_hmm,
                       Observation ot, Observation ot2, 
                       int t, DVector aqt, DVector aqt1, DVector bqt, int S,
                       Boolean twoDataFiles, LogDouble pr)
{
   int i,s,j,k,kk,m=0,mx,M=0,N,vSize,l;
   Vector mu_jm,var,mean=NULL,invk,otvs,otvs0;
   TMixRec *tmRec = NULL;
   float **outprob;
   Matrix inv;
   LogFloat c_jm=0,a,prob=0.0;
   LogDouble x,initx = LZERO;
   float zmean,zmeanlr,zmean2,tmp;
   double Lr,steSumLr,sumLr=0;
   HMMSet *hset;
   HSetKind hsKind;
   AlphaBeta *ab;
   ...
   if(hset->fMPE){
	   otvs0=ApplyFprojection(hdfInfo,t);
   }
   else{
	   otvs0=ot.fv[1];
   }
   ...
   for (j=2;j<N;j++) {
      if (fbInfo->maxM>1){
         initx = hmm->transP[1][j] + aqt[1];/* alpha(t=1,q,j=1), check htkbook@pp.144 alpha(t>1,q,j) */
         if (t>1)
            for (i=2;i<N;i++){
               a = hmm->transP[i][j];
               if (a>LSMALL)
                  initx = LAdd(initx,aqt1[i]+a);/* sum(i){alpha(t-1,q,i)*a(q,i,j)} */
            }
         /* so far initx=alpha(t,q,j)/outprob(t,q,j)=aqt[j]/outprob(t,q,j), therefore no need to subtract outprob(t,q,j)=logsum(m){cjm*Njm}, */
         initx += bqt[j] - pr;/* aqt[j]+bqt[j]-pr */
      }
      if ((trace&T_MIX) && (fbInfo->uFlags&UPMIXES) &&
          NonSkipRegion(fbInfo->skipstart,fbInfo->skipend,t))
         printf("  State %d: ",j);
      ...
      for (s=1;s<=S;s++,ste++){
            ...
            for (mx=1; mx<=M; mx++) {
               ...
               if (twoDataFiles){  
                  ...
                  /*otvs = ApplyCompFXForm(mp,ot.fv[s],inxform,&det,t);*/
            	   otvs = ApplyCompFXForm(mp,otvs0,inxform,&det,t);
               }

               wght = MixLogWeight(hset,al_hmm->owner->TVWR?me->weight:me->sweight);
         ...
         for (mx=1;mx<=M;mx++) { 
            ...
            case TIEDHS:
               ...
               swgt=wght=MixLogWeight(hset,ste->spdf.tpdf[m]);
               mp=tmRec->mixes[m];
               break;
            case DISCRETEHS:
               if (twoDataFiles)
                  ...
                  m=ot.vq[s];
               swgt=wght = 0.0; /* This is the log-weight, just for consistency! */
               mp=NULL;
               break;
            case PLAINHS:
            case SHAREDHS:
               m = mx;
               me = ste->spdf.cpdf+m;
               wght = MixLogWeight(hset,me->weight);/* get log(weight) */
               swgt = MixLogWeight(hset,me->sweight);/* get log(weight) */
               mp=me->mpdf;
               break;
            }
            ...
            if (swgt>LMINMIX){
              ...
              if (!mmix || (hsKind==DISCRETEHS)) /* For DISCRETEHS calcs are*/
                 x = aqt[j]+bqt[j]-pr;           /* same as single mix, compared to GMM case, i.e.initx=aqt[j]/outprob(t,q,j) */
              ...
              if (-x<pruneSetting.minFrwdP) {
      		    if (twoDataFiles){  /* switch to new data for mu & var est */
      	            ...
      	            if (hset->upVTS_NM||hset->upVTS_AM){
      	            	otvs = ApplyCompFXForm(mp,otvs0,fbInfo->inXForm,&det,t);
      	            }
      	            else{
      	            	otvs = ApplyCompFXForm(mp,otvs0,fbInfo->paXForm,&det,t);
      	            }
      		    }

                 Lr = exp(x);/* Rjm(t)=gamma(state=j,gaussian=m,ot)=P(state=j,gaussian=m|ot;theta) Gaussian posterior probability */
                  /* More diagnostics */
                  ...
                  */ 
                  if (fbInfo->uFlags&UPSEMIT) {
                     ...
                     if ((fbInfo->uFlags&UPMEANS) && (fbInfo->uFlags&UPVARS)) {
                        ...
                        va = (VaAcc *) GetHook(mp->cov.var);
                        ma->occ += Lr;/* denominator of its update formula */
                        va->occ += Lr;
                        mu_jm = ma->mu;
                        if ((mp->ckind==DIAGC)||(mp->ckind==INVDIAGC)){
                           ...
                           for (k=1;k<=vSize;k++) {
                              zmean=otvs[k]-mean[k];
                              zmeanlr=zmean*Lr;
                              mu_jm[k] += zmeanlr;/* accumulate Lr*(ot-ujm) so that m(p+1)=m(p)+mu */
                              var[k] += zmean*zmeanlr;/* accumulate Lr*(ot-ujm)(ot-ujm) */
                           }
                        } else {
                           inv = va->cov.inv;
                           for (k=1;k<=vSize;k++) {
                  ...
                  if ((fbInfo->uFlags&UPRWGTS) &&Lr>0 &&hset->TVWR){
                	  float iLr;
                	  rwa=(RWtAcc *) (me->rwgt->hook);
                	  HLink* synr=hdfInfo->synr;
                	  Boolean lNAT=hdfInfo->lNAT;
                	  short* regwsize=hset->pwidth;
                	  int offset=0;
                	  int _s,_t=t-hdfInfo->StartTime,c_s=(hset->temp_ctx_size+1)/2;
                	  int l_s=c_s-1,r_s=(c_s+1)>hset->temp_ctx_size?0:(c_s+1);
                	  int lctx_loc,rctx_loc;
                	  int *sparse_idx;
                	  Matrix* priors=hset->priors;
                	  Vector posVec;
                	  switch(hset->rkind){
                	  case NONE_CTX_REG:
                	  case TEMP_CTX_REG:
						  if(l_s>0){
							  lctx_loc=hdfInfo->ctxLoc[s][_t][l_s];
							  for(i=1;i<=regwsize[s];i++){
								  rwa->hjm[offset+i]+=Lr*hdfInfo->ctxPos[s][lctx_loc][i]/(priors?priors[s][l_s][i]:1)*me->rwgt->regparm[offset+i]/me->rwgt->wjmt[l_s];
							  }
							  offset+=regwsize[s];
						  }
						  if(c_s>0){
							  for(i=1;i<=regwsize[s];i++){
								  iLr=Lr*hdfInfo->frmPos[s][_t][i]/(priors?priors[s][c_s][i]:1)*me->rwgt->regparm[offset+i]/me->rwgt->wjmt[c_s];
								  rwa->hjm[offset+i]+=iLr;
								  if(lNAT&&iLr){
									  StreamElem *iste=(synr[i]->svec+2)->info->pdf+1;
									  MixtureElem* ime=iste->spdf.cpdf+1;
									  int im,iM=iste->nMix;
									  float *tempV=&(hdfInfo->loupMtx[_t][(i-1)*iM]);
									  WtAcc *iwa = (WtAcc *) iste->hook;
									  for(im=1;im<=iM;im++,ime++){
										  MixPDF* imp=ime->mpdf;
										  MuAcc *ima=(MuAcc *) GetHook(imp->mean);
										  VaAcc *iva=(VaAcc *) GetHook(imp->cov.inv);
										  float imLr=iLr*tempV[im];
										  ima->occ+=imLr;
										  iva->occ+=imLr;
										  iwa->c[im]+=imLr;
									  }
						  ...
						  if(r_s>0){
							  rctx_loc=hdfInfo->ctxLoc[s][_t][r_s];
							  for(i=1;i<=regwsize[s];i++){
								  rwa->hjm[offset+i]+=Lr*hdfInfo->ctxPos[s][rctx_loc][i]/(priors?priors[s][r_s][i]:1)*me->rwgt->regparm[offset+i]/me->rwgt->wjmt[r_s];
							  }
							  offset+=regwsize[s];
						  }
                		  break;
                	  case SPAT_CTX_REG:
						  for(_s=1;_s<=regwsize[0];_s++){
							  for(i=1;i<=regwsize[_s];i++){
								  rwa->hjm[offset+i]+=Lr*hdfInfo->frmPos[_s][_t][i]/(priors?priors[_s][1][i]:1)*me->rwgt->regparm[offset+i]/me->rwgt->wjmt[_s];
							  }
							  offset+=regwsize[_s];
						  }
                		  break;
                	  case SPTP_CTX_REG:
						  for(_s=1;_s<=regwsize[0];_s++){
							  /*left*/
							  lctx_loc=hdfInfo->ctxLoc[_s][_t][l_s];
							  for(i=1;i<=regwsize[_s];i++){
								  rwa->hjm[offset+i]+=Lr*hdfInfo->ctxPos[_s][lctx_loc][i]/(priors?priors[_s][1][i]:1)*me->rwgt->regparm[offset+i]/me->rwgt->wjmt[(_s-1)*hset->temp_ctx_size+1];
							  }
							  offset+=regwsize[_s];
							  /*center*/
							  for(i=1;i<=regwsize[_s];i++){
								  rwa->hjm[offset+i]+=Lr*hdfInfo->frmPos[_s][_t][i]/(priors?priors[_s][2][i]:1)*me->rwgt->regparm[offset+i]/me->rwgt->wjmt[(_s-1)*hset->temp_ctx_size+2];
							  }
							  offset+=regwsize[_s];
							  /*right*/
							  rctx_loc=hdfInfo->ctxLoc[_s][_t][r_s];
							  for(i=1;i<=regwsize[_s];i++){
								  rwa->hjm[offset+i]+=Lr*hdfInfo->ctxPos[_s][rctx_loc][i]/(priors?priors[_s][3][i]:1)*me->rwgt->regparm[offset+i]/me->rwgt->wjmt[(_s-1)*hset->temp_ctx_size+3];
							  }
							  offset+=regwsize[_s];
						  }
                		  break;
                	  ...
                	  case TEMP_SPA_REG:
						  if(l_s>0){
							  ...
							  for(i=1;i<=sparse_idx[0];i++){
								  rwa->hjm[offset+i]+=Lr*hdfInfo->ctxPos[s][lctx_loc][sparse_idx[i]]/(priors?priors[s][l_s][sparse_idx[i]]:1)*me->rwgt->regparm[offset+i]/me->rwgt->wjmt[l_s];
							  }
							  offset+=sparse_idx[0];
						  }
						  if(c_s>0){
							  ...
							  for(i=1;i<=sparse_idx[0];i++){
								  rwa->hjm[offset+i]+=Lr*hdfInfo->frmPos[s][_t][sparse_idx[i]]/(priors?priors[s][c_s][sparse_idx[i]]:1)*me->rwgt->regparm[offset+i]/me->rwgt->wjmt[c_s];
							  }
							  offset+=sparse_idx[0];
						  }
						  if(r_s>0){
							  ...
							  for(i=1;i<=sparse_idx[0];i++){
								  rwa->hjm[offset+i]+=Lr*hdfInfo->ctxPos[s][rctx_loc][sparse_idx[i]]/(priors?priors[s][r_s][sparse_idx[i]]:1)*me->rwgt->regparm[offset+i]/me->rwgt->wjmt[r_s];
							  }
						  }
                		  break;
                	  }
            		  rwa->occ+=Lr;
                	  wa->c[m]+=Lr;/* state/component occupancy count */
                  }
...
{
   int q,t,start,end,negs,i;
   DVector aqt,aqt1,bqt,bqt1,bq1t;
   float chksum=0;
   HLink al_hmm, up_hmm;
   AlphaBeta *ab;
   /*HMMSet *hset=fbInfo->up_hset;*/
   ...
   InitAlpha(ab,&start,&end,utt->Q,fbInfo->skipstart,fbInfo->skipend);/* alpha@t=1 */
   ab->occa = NULL;
   if (trace&T_OCC) 
      CreateTraceOcc(ab,utt);
   for (q=1;q<=utt->Q;q++){             /* inc access counters */
      up_hmm = ab->up_qList[q];
      negs = (int)up_hmm->hook+1;		/* zero negs means this phone unit doesn't exist in the training data*/
      up_hmm->hook = (void *)negs;		/* the occurrence of specific hmm or phone unit, since its state is tied, this number is not consistent to the summation of respective states*/
   }
   ...
   for (t=1;t<=utt->T;t++) {
      ...
      /*reset synhset va occ count, for each time instance*/
	  HDFInfo* hdfInfo=&(fbInfo->hdfInfo);
	  HLink* synr=hdfInfo->synr;
	  Boolean lNAT=hdfInfo->lNAT;
	  if(lNAT){
		  for(i=1;i<=hdfInfo->hset->regwSize;i++){/* each monophone is with single state */
			  StreamElem *iste=(synr[i]->svec+2)->info->pdf+1;
			  MixtureElem* ime=iste->spdf.cpdf+1;
			  int im,iM=iste->nMix;
			  for(im=1;im<=iM;im++,ime++){
	  ...
	  chksum=0;
      for (q=start;q<=end;q++) { /* scan from arc with lower q */
         ...
         aqt1 = (t==1)      ? NULL:ab->alphat1[q];/* alpha[t-1] */
         bq1t = (q==utt->Q) ? NULL:ab->beta[t][q+1];
         SetOcct(al_hmm,q,ab->occt,ab->occa,aqt,bqt,bq1t,utt->pr);/* calculate gamma(t, q, j)*/
         /* accumulate the statistics */
         if (fbInfo->uFlags&(UPMEANS|UPVARS|UPMIXES|UPXFORM|UPRWGTS))
            chksum+=UpMixParms(fbInfo,q,up_hmm,al_hmm,utt->ot,utt->ot2,t,aqt,aqt1,bqt,
                       utt->S, utt->twoDataFiles, utt->pr);
         if (fbInfo->uFlags&UPTRANS)
            UpTranParms(fbInfo,up_hmm,t,q,aqt,bqt,bqt1,bq1t,utt->pr);
      }
      /*accumulate synhset mu occ count for each time instance to avoid duplicate calculation of square acc for saving time*/
      if(lNAT){
    	  Vector lobs=hdfInfo->lobsMtx[t];
    	  int lvSize=hdfInfo->synhset->vecSize;
    	  float chksum=0,temp;
    	  int im, iM, k,kk,tempk,tempkk,N=hdfInfo->hset->regwSize;
    	  Vector Pdif=CreateVector(&gstack,lvSize);
    	  Vector dif=CreateVector(&gstack,lvSize);
    	  Matrix tempX=CreateMatrix(&gstack,N,8);
    	  for(i=1;i<=N;i++){
    		  StreamElem *iste=(synr[i]->svec+2)->info->pdf+1;
    		  MixtureElem* ime=iste->spdf.cpdf+1;
    		  iM=iste->nMix;
    		  for(im=1;im<=iM;im++,ime++){
    			  MixPDF* imp=ime->mpdf;
    			  SVector mu=imp->mean;
    			  STriMat inv=imp->cov.inv;
    			  MuAcc *ima=(MuAcc *) GetHook(mu);
    			  VaAcc *iva=(VaAcc *) GetHook(inv);
    			  float imLr=iva->occ;/*store the occupancy for the current time instance*/
    			  chksum+=imLr;
    			  tempX[i][im]=imLr;
    			  if(imLr<0.001){
    				  continue;
    			  ...
    			  /*for(k=1;k<=lvSize;k++){
    				  Pdif[k]=0;
    				  for(kk=1;kk<=lvSize;kk++){
    					  temp=k>kk?inv[k][kk]:inv[kk][k];
    					  Pdif[k]+=temp*dif[kk];
    				  }
    			  }
    			  for(k=1;k<=lvSize;k++){
    				  ima->mu[k]+=imLr*dif[k];
    				  for(kk=1;kk<=k;kk++){
    					  iva->cov.inv[k][kk]+=imLr*(inv[k][kk]-Pdif[k]*Pdif[kk]);
    				  }
    			  }*/
    			  for(k=1;k<=lvSize;k++){
    				  ima->mu[k]+=imLr*dif[k];
    				  for(kk=1;kk<=k;kk++){
    					  iva->cov.inv[k][kk]+=imLr*dif[k]*dif[kk];
    				  }
    			  }
    			  iva->occ=ima->occ;
    		  }
    	  }
    	  if(chksum>=1.01||chksum<=0.99){
    		  printf("bad occupancy check sum %f\n",chksum);
    		  fflush(stdout);
    	  }
    	  FreeVector(&gstack,Pdif);
      }
              ...
              char *labDir, char *labExt)
{
   ...
   utt->tr = LOpen(&utt->transStack,labfn,lff);/* load the transcription and return it */
   utt->Q  = CountLabs(utt->tr->head);/* count the number of labs for this utterance */
   if (utt->Q==0)
                         ...
                         char * datafn, int * maxMixInS)
{
   ...
   utt->ot = MakeObservation(&gstack,al_hset->swidth,info.tgtPK,
                             al_hset->hsKind==DISCRETEHS,eSep);/* make the observation containing everything from data file */
   if (utt->twoDataFiles)
