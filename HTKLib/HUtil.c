...
RegWgt *CloneRegWgt(HMMSet *hset, RegWgt *s, Boolean sharing){
	RegWgt *t;
	if (s->nUse>0 && sharing){
		++s->nUse;
		return s;
	}
	t = (RegWgt*)New(hset->hmem,sizeof(RegWgt));
	t->regparm= CloneSVector(hset->hmem,s->regparm,sharing);
	t->nUse=0;
	t->hook=0;
	t->cached_t=-1;
	t->rIdx=-1;
...
MixtureVector CloneStream(HMMSet *hset, StreamElem *ste, Boolean sharing)
{
   ...
   if (hset->hsKind == PLAINHS || hset->hsKind == SHAREDHS){
      ...
      for (m=1; m<=M; m++,sme++,tme++){
         ...
         tme->rwgt=sme->rwgt?CloneRegWgt(hset->hmem,sme->rwgt,sharing):NULL;
      }
...
RegWgt** CloneTStream(HMMSet *hset, TStreamElem *ste, Boolean sharing)
{
   int m,M=ste->nMix;
   RegWgt** mv=(RegWgt**)New(hset->hmem, sizeof(RegWgt*)*(M+1));
   M = ste->nMix;
   for (m=1; m<=M; m++){
	   mv[m]=CloneRegWgt(hset,ste->rwgts[m],sharing);
   }
   return mv;
}
...
TVWRInfo *CloneTVWR(HMMSet *hset, TVWRInfo *szi, Boolean sharing)
{
   TVWRInfo *tzi;  /* the target */
   int s;

   if (szi->nUse>0 && sharing) {
      ++szi->nUse;
      return szi;
   }
   tzi = (TVWRInfo *)New(hset->hmem,sizeof(TVWRInfo));
   tzi->nUse = 0; tzi->hook = NULL;
   tzi->tstr = (TStreamElem*)New(hset->hmem,(hset->swidth[0]+1)*sizeof(TStreamElem));
   for(s=1;s<=hset->swidth[0];s++){
	   tzi->tstr[s].nMix=szi->tstr[s].nMix;
	   tzi->tstr[s].rwgts=CloneTStream(hset, &szi->tstr[s],sharing);
   }
   return tzi;
}
...
void CloneHMM(HLink src, HLink tgt, Boolean sharing)
{
   ...
   if(src->zvec){
	   zt = (TVWRElem *)New(hset->hmem,(tgt->numStates-2)*sizeof(TVWRElem));
	   tgt->zvec = zt-2; zs = src->zvec+2;
	   for (i=2; i<tgt->numStates; i++,zs++,zt++)
	      zt->info = CloneTVWR(hset,zs->info,sharing);
   }
   ...
   for (;hss->h<MACHASHSIZE;hss->h++)
      for (mac=((mac==NULL)?hss->hset->mtab[hss->h]:mac);
         ...
         if (mac->type == 'h') {
            ...
            /*char *name=HMMPhysName(hss->hset,hss->hmm);*/
            hss->N = hss->hmm->numStates;
            hss->se = hss->hmm->svec+2; hss->i=2;
            hss->si = hss->se->info;
            hss->ste = hss->si->pdf+1; hss->s=1;
            if(hss->hset->tiedTVWR){
                hss->ze = hss->hmm->zvec+2;
                hss->zi = hss->ze->info;
                hss->tte= hss->zi->tstr+1;
                hss->rwgt=hss->tte->rwgts[1];
            }
            else{
            	hss->ze = NULL;
            	hss->zi = NULL;
...
/* EXPORT->GoNextTVWR: move to next unseen state */
Boolean GoNextTVWR(HMMScanState *hss, Boolean noSkip)
{
   Boolean stepping = FALSE, ok = TRUE;
   int M;

   while (IsSeen(hss->zi->nUse) && ok){
      if (hss->i < hss->N-1) {
         ++hss->i; ++hss->ze; stepping = TRUE;
         hss->zi = hss->ze->info;
      } else if (noSkip)
         return FALSE;
      else{
...
/* EXPORT->GoNextStream: move to next unseen stream */
Boolean GoNextTStream(HMMScanState *hss, Boolean noSkip)
{
   Boolean stepping = FALSE, ok = TRUE;
   int M;

   while (IsSeen(hss->tte->nMix) && ok){
      if (hss->s < hss->S) {
         ++hss->s; ++hss->tte; stepping = TRUE;
      } else if (noSkip)
         return FALSE;
      else{
         stepping = FALSE;
         ok = GoNextTVWR(hss,FALSE);
      }
   }
   if (ok) {
      Touch(&hss->tte->nMix);
...
/* EXPORT->GoNextRegWgt: move to next unseen mixture component */
Boolean GoNextRegWgt(HMMScanState *hss, Boolean noSkip)
{
   Boolean ok = TRUE;

   while (IsSeen(hss->rwgt->nUse) && ok){
      if (hss->m < hss->M) {
         hss->rwgt=hss->tte->rwgts[++hss->m];
      } else if (noSkip)
         return FALSE;
      else
         ok = GoNextTStream(hss,FALSE);
   }
   if (ok) {
      Touch(&hss->rwgt->nUse);
      return TRUE;
...
void ConvLogWt(HMMSet *hset)
{
   ...
   NewHMMScan(hset, &hss);
   while (GoNextMix(&hss,FALSE)){
	   hss.me->sweight=hss.me->weight = MixLogWeight(hset,hss.me->weight);
   }
   EndHMMScan(&hss);
   hset->logWt = TRUE;
}
...
void ConvExpWt(HMMSet *hset)
{
   ...
   NewHMMScan(hset, &hss);
   while (GoNextMix(&hss,FALSE)){
      hss.me->weight = exp(hss.me->sweight);
   }
   EndHMMScan(&hss);
   hset->logWt = FALSE;
}
...
int MonoPhoneName(char* triphone, char* monophone){
	int i=0,j=0;
	char ch;
	while((ch=triphone[i++])!='\0'){
		switch(ch){
		case '-':
			j=0;break;
		case '+':
			monophone[j++]='\0';return j;
		default:
			monophone[j++]=ch;break;
		}
	}
	monophone[j++]='\0';
	return j;
...
static char *GetAlpha(char *s)
{
   ...
   for (i=0; i<MAXSTRLEN ; i++){
      ...
      if (ch==ESCAPE_CHAR) {
         ReadCh();
         /*if (ch<'0' || ch>'7') { this is the old code, should be wrong as we only interpret it as octal if it is followed by 0~7*/
         if (ch>='0' && ch<='7') {
            n = ch - '0'; ReadCh();
...
typedef enum {
   TRANSP_KEY, STATE_KEY, TVWR_KEY, DUR_KEY, WEIGHTS_KEY,REGW_KEY,
   MIX_KEY, MEAN_KEY, STREAM_KEY, COV_KEY
} Keyword;

static char *keymap[] = {
   "TRANSP", "STATE", "TVWR", "DUR", "WEIGHTS","REGW",
   "MIX", "MEAN", "STREAM", "COV"
};
                 ...
                 IntSet states, IntSet streams,HMMSet *hset)
{
   ...
   if (ch == '.') {
      ...
      case COV_KEY:
         ...
         break;
      default:
         EdError("Mean or Cov expected");break;
      }
   } else
      ChkType('m',type);
   for (h=models; h!=NULL; h=h->next) {
                       ...
                       IntSet states, HMMSet *hset)
{
   ...
   case WEIGHTS_KEY:
      ...
      break;
   default:
      EdError("dur, weight, stream or mix expected");break;
   }
}
...
/* PState: parse TVWR and add all matches in models to ilist */
static void PTVWR(ILink models, ILink *ilist, char *type, HMMSet *hset)
{
   IntSet states;
   int j;
   HMMDef *hmm;
   ...
   SkipSpaces();
   if (ch == '.') {
      HError(1,"Unsupported sub-item for TVWR element");
   } else {
      ChkType('z',type);
      for (h=models; h!=NULL; h=h->next) {
         hmm = h->owner;
...
static void PItemSet(ILink *ilist, char *type, HMMSet *hset)
{
   ...
   if (ch == '.') {             /* look for subcomponents */
      ...
      case STATE_KEY:  
         ...
         break;
      case TVWR_KEY:
    	  PTVWR(models,ilist,type,hset);
    	  break;
      default: 
         EdError("State, TVWR or TransP expected");break;
      }
      FreeItems(&models);
...
Ptr GetMacroHook(MLink ml)
{
   ...
   case 'q': /* RegWgt* */
	  hook=((RegWgt *)(ml->structure))->hook; break;
   case 'c': /* STriMat */
   case 'i': /* STriMat */
   case 'x': /* SMatrix */
   case 't': /* SMatrix */
   ...
   default:
      hook=NULL;
      HError(7270,"GetMacroHook: Getting hook of non-existant macro");break;
   }
   return(hook);
}
...
void SetMacroHook(MLink ml,Ptr hook)
{
   ...
   case 'z':
	  ((TVWRInfo *)(ml->structure))->hook=hook; break;
   case 'm': /* MixPDF * */
      ((MixPDF *)(ml->structure))->hook=hook; break;
   case 'q': /* RegWgt * */
	  ((RegWgt *)(ml->structure))->hook=hook; break;
   case 'c': /* STriMat */
   case 'i': /* STriMat */
   case 'x': /* SMatrix */
   case 't': /* SMatrix */
...
int GetMacroUse(MLink ml)
{
   ...
   case 'z':
      use=((TVWRInfo *)(ml->structure))->nUse; break;
   case 'm': /* MixPDF * */
      use=((MixPDF *)(ml->structure))->nUse; break;
   case 'j': /* InputXForm * */
      use=((InputXForm *)(ml->structure))->nUse; break;
   case 'e':
      use=((Fprojection*)(ml->structure))->nUse; break;
   case 'q':/* RegWgt* */
	  use=((RegWgt *)(ml->structure))->nUse; break;
   case 'c': /* STriMat */
   case 'i': /* STriMat */
   case 'x': /* SMatrix */
   case 't': /* SMatrix */
   ...
   default:
      use=-1;
      HError(7270,"GetMacroUse: Getting use of non-existant macro");break;
   }
   return(use);
}
...
void SetMacroUse(MLink ml,int use)
{
   ...
   case 'z':
      ((TVWRInfo *)(ml->structure))->nUse=use; break;
   case 'm': /* MixPDF * */
      ((MixPDF *)(ml->structure))->nUse=use; break;
   case 'j': /* InputXForm * */
      ((InputXForm *)(ml->structure))->nUse=use; break;
   case 'e':
	  ((Fprojection*)(ml->structure))->nUse=use; break;
   case 'q':
	   ((RegWgt *)(ml->structure))->nUse=use; break;
   case 'c': /* STriMat */
   case 'i': /* STriMat */
   case 'x': /* SMatrix */
   case 't': /* SMatrix */
   ...
   case 'r': 
      break;
   default:
      HError(7270,"SetMacroUse: Setting use of non-existant macro");break;
   }
}
...
void LoadStatsFile(char *statfile,HMMSet *hset,Boolean otrace)
{
   ...
   while(ReadInt(&src,&idx,1,bin)) {
      ...
      for (i=2; i<N; i++) {
         ...
         if(hmm->zvec){
             zi = hmm->zvec[i].info;
             zi->stateCounter = count;/* load the # of times the state occurred */
             memcpy(&(zi->hook),&x,sizeof(float)); /* !! */
         }
         occSum += x; ++occN;
      }
   }
