...
void InitLabel(void)
{
   ...
   if (numParm>0){
      ...
      if (GetConfBool(cParm,numParm,"PURELABELS",&b))  pureLabs = b;
   }
}
   ...
   *isMLF = FALSE; 
   ...
   for (e=(q==NULL?mlfHead:q); e != NULL; e = (e==NULL?mlfHead:e->next)) {
      ...
      case PAT_GENERAL:
         ...
         isMatch = DoMatch(fname,e->pattern); /* search the matched lab file according to the pattern */
         break;
      case PAT_ANYPATH:
         if (trace&T_MAT) 
      ...
      if ( isMatch ) {
         if (e->type == MLF_IMMEDIATE) {
            f = mlfile[e->def.immed.fidx];/* usually only one file containing all the labs is opened */
            if (fseek(f,e->def.immed.offset,SEEK_SET) != 0) /* different lab file has different offset for searching */
...
static void SaveHTKLabels( FILE *f, Transcription *t) 
{
   ...
   for (i=1; i<=t->numLists; i++,ll=ll->next){
      ...
      for (p = hd->succ; p->succ != NULL; p = p->succ) {
         if (p->start>=0.0&&!pureLabs) {
            fprintf(f,"%.0f ",p->start / htkLabelTimeScale);
            if (p->end>=0.0) 
               fprintf(f,"%.0f ",p->end / htkLabelTimeScale);
            int start=ceil(p->start/1E+5 -0.5);
            int end=ceil(p->end/1E+5 -0.5);
            for(k=start;k<end;k++){
            	/*printf("[%d]=%f\n",frame,p->finfo[k]);*/
            	fprintf(tmp,"%f\n",p->aux[k-start]);
            }
