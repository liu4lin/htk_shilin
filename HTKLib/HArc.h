...
typedef struct _MTEStruct{
  float correctness;  /* for inexact MPE. */
  float alphaError, betaPlusError; /*for MPE training.*/

  float diff;
  float alphaDiff, betaPlusDiff;
  CorrN *cn; /* if doing dynamic MPE. */
} MTEStruct;
...
#define GHOST_ARC -1
  ...
  /*MTE case, state alignment required*/
  IntVec slAlign;   /* Array[1...T] 3 state[2 3 4] alignments of each records in phone*/
};
...
void AttachMTEInfo(ArcInfo *aInfo); /* attaches the "mpe" fields  */
