...
typedef struct{  /* R.E MPE code, these structures store all correct-lattice arcs active at time t */
   ...
   IntVec slAlign;   /* for MET use, [1...T] 3 state alignments of each records in word*/
   HLink hmm;
} CorrectArc;
...
/**
 * a transition probability, b emitting probability, both are logged
 */
static void Viterbi(MemHeap *x, Matrix a, Matrix b, IntVec qbest, int N, int T){
    float** v=(float**)New(x,sizeof(float*)*(N+1));
    int** qmax=(int**)New(x,sizeof(int*)*(N+1));
    int t, i, j, best_i;
    float max_prob;
    for(j=1;j<=N;j++){
    	v[j]=(float*)New(x,sizeof(float)*(T+1));
    	qmax[j]=(int*)New(x,sizeof(int)*(T+1));
    }
    ...
    for(t=1;t<=T;t++){
    	for(j=1;j<=N;j++){
    		best_i=-1;
    		max_prob=LZERO;
    		for(i=1;i<=N;i++){/*find the best previous partial alignment*/
    			double lgprob=v[i][t-1]+b[j][t]+a[i][j];/*logged likelihood*/
    			if(lgprob>max_prob){
    				max_prob=lgprob;
    				best_i=i;
    			}
    		}
    		v[j][t]=max_prob;/*store the best path likelihood*/
    		qmax[j][t]=best_i;/*store the backtracking state information*/
    	}
    }
    /*the ending boundary doesn't have the emitting probability,
     * in real application, the ending previous best state has only a possible state
     * only state 4 has nonzero transition probability to state 5*/
	best_i=-1;
	max_prob=LZERO;
    for(i=1;i<=N;i++){
		double lgprob=v[i][T]+a[i][N];
		if(lgprob>max_prob){
			max_prob=lgprob;
			best_i=i;
		}
	}
    /*to obtain the last best state, it's necessary to run the ending boundary FindArgMax,
     * However, the typical last state can only be state 4*/
    qbest[T]=best_i;
    for(t=T-1;t>=1;t--)
    	qbest[t]=qmax[qbest[t+1]][t+1];/*qmax always stores the previous best state*/
    Dispose(x,v);
}
/* ShStrP: Stream Outp calculation exploiting sharing */
static float Outp(Vector v, int t, StreamElem *ste, AdaptXForm *xform, float *bestComp, int s)
{
   if(fbInfo->hset->fMPE){
 	  v=ApplyFprojection(&fbInfo->hdfInfo,t);
   }
   if(fbInfo->hset->TVWR){
 	  ApplyWprojection(ste, &fbInfo->hdfInfo, t, s);
   }
   MixtureElem *me;
   MixPDF *mp;
   int m,M;
   PreComp *pMix;
   LogFloat det,x, best_p, mixp, temp;
   LogFloat wgt,swgt;
   M = ste->nMix;
   me = ste->spdf.cpdf+1;
   if (M==1){                 /* Single Mix Case */
      ...
      if (pMix->time == t)
         ...
         x += det;
         x += MixLogWeight(fbInfo->hset,me->weight);
      ...
      *bestComp=1;
      ...
      x= LZERO;
      for (m=1;m<=M;m++,me++) {
    	 wgt=MixLogWeight(fbInfo->hset,me->weight);
    	 swgt=MixLogWeight(fbInfo->hset,me->sweight);
         if (swgt>LMINMIX){
        	 mp = me->mpdf;
        	 pMix = (PreComp *)mp->hook;
        	 if (pMix->time==t)
...
static HLink GetHLink(FBLatInfo *fbInfo, LabId label){
	MLink ml;
    ml = FindMacroName (fbInfo->hset, 'l', label);
    if (!ml)
    	HError (9999, "cannot find model for label '%s'", label->name);
    HLink hmm=((HLink) ml->structure);
    return hmm;
}

static void ForceAlignment(FBLatInfo *fbInfo, HLink hmm, IntVec slAlign, int i_start, int i_end, int T){
    int N=hmm->numStates;/*include*/
    Matrix a=hmm->transP;
    Matrix b=CreateMatrix(&fbInfo->tempStack, N, T);
    Matrix best_comps=CreateMatrix(&fbInfo->tempStack,N,T);
    IntVec qbest=slAlign;
    ResetObsCache();
    int j,t,rev_t;
    StreamElem *ste;
    for(t=i_start;t<=i_end;t++){
    	...
    	for(j=2;j<N;j++){
             ...
             if(fbInfo->hset->tiedTVWR){
            	 UpdateRegWgts(ste, hmm->zvec[j].info, 1);
             }
             b[j][rev_t] = Outp(fbInfo->al_ot.fv[1], t+StartTime, ste, fbInfo->inXForm, &best_comps[j][rev_t], 1);
    	}
    }
    Viterbi(&fbInfo->tempStack, a, b, qbest, N, T);
    for(t=1;t<=T;t++){
    	int best_state=qbest[t];
    	int best_comp=best_comps[best_state][t];
    	qbest[t]=best_comp*10+best_state;
    }
}
...
static void StepAlpha(int t)
{
   ...
   for (q = fbInfo->aInfo->qLo[t]; q <= fbInfo->aInfo->qHi[t]; q++) { /*This is just to avoid iterating over all q's.*/
      ...
      if(t >= ac->t_start && t<=ac->t_end){ /*If it's in the beam...*/
         ...
         if(t==ac->t_start) aq[1] = ac->locc - ac->aclike;/*if aq[1]=0, aq[Nq]=ac->aclike when t=ac->t_end*/
...
/* ShStrP: Stream Outp calculation exploiting sharing */
static float * ShStrP(Vector v, int t, StreamElem *ste, AdaptXForm *xform, MemHeap *amem, int s)
{
	WtAcc *wa;
	MixtureElem *me;
	MixPDF *mp;
	...
	int m,M;
	PreComp *pMix;
	LogFloat det, x, mixp, wt, swt;
	wa = (WtAcc *)ste->hook;
	if (wa->time==t&&!fbInfo->hset->tiedTVWR)           /* seen this state before */
		outprobjs = wa->prob;
	else {
		if(fbInfo->hset->fMPE){
			v=ApplyFprojection(&fbInfo->hdfInfo,t);
		...
		if (M==1){                 /* Single Mix Case */
			...
			x = LZERO;
			for (m=1;m<=M;m++,me++) {
				wt = MixLogWeight(fbInfo->hset,me->weight);
				swt = MixLogWeight(fbInfo->hset,me->sweight);
				if (swt>LMINMIX){
					mp = me->mpdf;
					pMix = (PreComp *)mp->hook;
					if (pMix->time==t)
      ...
      if(t>=fbInfo->aInfo->ac[q].t_start && t<=fbInfo->aInfo->ac[q].t_end) { /* HMM is active at this time... */
         ...
         for (j=2;j<Nq;j++){
            ...
            for (s=1;s<=fbInfo->S;s++,ste++){
               if(fbInfo->hset->tiedTVWR){
            	   UpdateRegWgts(ste, hmm->zvec[j].info, s);
               }
               ...
               case SHAREDHS:
            	   if (fbInfo->S==1)
            		   outprob[j][0] = ShStrP(fbInfo->al_ot.fv[s],t+StartTime,ste,fbInfo->inXForm,fbInfo->aInfo->mem, s);
            	   else
            		   outprob[j][s] = ShStrP(fbInfo->al_ot.fv[s],t+StartTime,ste,fbInfo->inXForm,fbInfo->aInfo->mem, s);
            	   break;
               default:       HError(1, "Unknown hset kind.");break;
               }
               if (fbInfo->S==1)
...
/*within arc calculation, to calculate P(O,t=t_start...t_end|q), arc likelihood */
void SetModelBetaPlus(int t, int q){
   ...
   for(i=2;i<Nq;i++){
      x = bqt[Nq] + hmm->transP[i][Nq];/*initial term shared by t=T and t<T, where T=t_end.*/
      if(t+1<=ac->t_end){ /*in beam next time frame, if t<T, more terms are added to bqt[i]*/
         bqt1=ac->betaPlus[t+1];/*only when t<T, t+1 can be still within this arc*/
         for(j=2;j<Nq;j++)
            x = LAdd(x, bqt1[j] + hmm->transP[i][j]);
      }
      x += outprob[i][0][0];
      bqt[i] = x;/*betaPlus(t, q, s=i)=beta(t, q, s=i)*outprob(t, q, s=i)*/
   }
   x=LZERO;
   for(i=2;i<Nq;i++) x = LAdd(x, bqt[i]+hmm->transP[1][i]);
   bqt[1] = x;/*betaPlus(t, q, s=1)=beta(t, q, s=1), since no more outprob after the summation*/
}
...
static void SetBetaPlus()
{
   ...
   LogDouble x;
   /* calculate the betaPlus(t,q,state=i)=beta(t,q,state=i)+outprob(t,state=i) */
   /* when t==ac->t_start, accumulate the arc likelihood P(O,t=t_start...t_end|q)*/
   /* 
      Columns T-1 -> 1.
   */
   ResetObsCache();  
   for (t=fbInfo->T;t>=1;t--) {
      ...
      for (q=fbInfo->aInfo->qHi[t];q>=fbInfo->aInfo->qLo[t];q--) { /*MAX(qHi[t],qLo[t]) because of the case for tee models where qHi[t]=qLo[t]-1 .*/
         ...
         if(t>=ac->t_start && t<=ac->t_end){ /*in beam.*/
            SetModelBetaPlus(t,q);/*a backward probability calculation during t=t_start...t_end*/
         }
         if(t==ac->t_start){ /* We need to set "aclike", the total accumulated acoustic
            ...
            ac->aclike = x;/* within-arc "forward" probability @(state=5, t=t_time)
             	 	 	 	 or "backward" probability @(state=1, t=t_time) */
         }	
      }
   }
}
...
void UpSkipTranParms(int q, int t){
   ...
   HLink hmm=ac->hmm;
   double occ = ac->locc;
   float mee_acc_scale = fbInfo->AccScale*(fbInfo->MPE?ac->mpe_occscale:1);
...
static void UpTranParms(int t, int q){ 
   ...
   Acoustic *ac = fbInfo->aInfo->ac+q;
   HLink hmm = ac->hmm;
   float x=0,mee_acc_scale = fbInfo->AccScale*(fbInfo->MPE?ac->mpe_occscale:1);
   int local_accindx = (mee_acc_scale > 0 ? fbInfo->num_index : fbInfo->den_index);
   if(mee_acc_scale>0){
  	   mee_acc_scale +=   fbInfo->AccScale * (fbInfo->MPE? ac->mpe_occscale_dif: 0);
   }
   float abs_mee_acc_scale = fabs(mee_acc_scale);
...
/* Iterates over all active mpdf's and actually accumulates stats. */
void DoAllMixUpdates(int t){
   int s,m,i,j,k,vSize;
   MixPDF *mp;
   float Lr, unscaledLr, LrWithSign;
   Vector mean,variance,up_otvs, up_otvs0, al_otvs,mu_jm;
   LogFloat det;
   ...
   VaAcc *va,*vammi;
   int local_accindx;
   Fprojection* fp=fbInfo->hset->fproj;
   int targetDim=fp?fp->targetDim:0;

   Vector diff_t=NULL;/* = (Matrix*)New(&fbInfo->tempStack, sizeof(Matrix)*(1+ncls));*/

   if(fbInfo->hset->fMPE&&fbInfo->MPE){
	  diff_t=CreateVector(&fbInfo->tempStack,targetDim);
	  ZeroVector(diff_t);
   }
   for(s=1;s<=fbInfo->S;s++){
      float steSumLr = 0.0;
      ...
      up_otvs0=fbInfo->al_ot.fv[s];
      if(fbInfo->hset->fMPE){
     	 up_otvs0=ApplyFprojection(&fbInfo->hdfInfo, t+StartTime);
      }
      for(m=0;m<nPDFs[s];m++){
         ...
         if(LrWithSign>0.0){
        	 local_accindx = fbInfo->num_index;/*numerator index for MuAcc */
         }
         else{
        	 local_accindx = fbInfo->den_index;/*denominator index for VaAcc */
         }
         Lr = fabs(LrWithSign);
         ...
         if (fbInfo->twoDataFiles){
             ...
             up_otvs = ApplyCompFXForm(mp,up_otvs0,fbInfo->paXForm,&det,t);
         } 
         ...
         /* -------------------- fMPE updates  --------------------*/
		 if(fbInfo->uFlags&UPFPROJ){
			 if(fbInfo->MPE){/*direct differential, MPE criteria*/
				 for(i=1;i<=targetDim;i++){
					 float ivar=mp->ckind == INVDIAGC?variance[i]:1/variance[i];
					 diff_t[i]+=LrWithSign*(mean[i]-up_otvs[i])*ivar;
				 }
			 }
			 else{/*indirect differential, ML update*/
				 ma=((MuAcc*)GetHook(mean))+local_accindx;
				 va=((VaAcc*)GetHook(variance))+local_accindx;
				 for(i=1;i<=targetDim;i++){
					 fbInfo->hdfInfo.indiff_cache[t][i]+=(Lr/ma->occ)*(ma->udif[i]+2*va->vdif[i]*(up_otvs[i]-mean[i]));
				 }
			 }
		 }
         /* -------------------- (c) Std Mixture updates --------------------*/
         else if (fbInfo->uFlags&UPMEANS) { /* cant update vars but not means. */
            ...
            if (fbInfo->uFlags&UPVARS){ /* This code is longer than it has to be, to reduce if-statements within loops. */
               ...
               case FULLC:
                  ...
                  break;
               default: HError(1, "Unknown CKIND.");break;
               }
            } else {
               ma->occ += Lr;
               for (k=1;k<=vSize;k++) {
      ...
      if(steSumLr > 1.01 || steSumLr < 0.99)
    	  HError(-1, "Wrong steSumLr: %f, t=%d, s=%d",steSumLr, t, s);
   }
   if((fbInfo->uFlags&UPFPROJ)&&fbInfo->MPE){
	   DAcc* dAcc=(DAcc*)fbInfo->hset->dAcc;
	   Vector indiff_t=fbInfo->hdfInfo.indiff_cache[t];
	   IdxLink* idx=fbInfo->hdfInfo.nzIdx4fMPE[t];
	   for(;idx!=NULL;idx=idx->next){
		   j=idx->idx;
		   float f_tj=idx->val;
		   for(i=1;i<=fp->targetDim;i++){
			   float dij=(diff_t[i]+indiff_t[i])*f_tj;
			   dAcc->sumSq[i][j]+=fabs(dij);
			   dAcc->sqSum[i][j]+=dij*dij;
			   if(dij>0){
				   dAcc->pGradient[i][j]+=dij;
			   }
			   else{
				   dAcc->nGradient[i][j]-=dij;
			   }
		   }
	   }
	   for(i=1;i<=fp->targetDim;i++){
		   dAcc->check1[i][1]+=diff_t[i];
		   dAcc->check1[i][2]+=indiff_t[i];
		   dAcc->check2[i][1]+=diff_t[i]*up_otvs[i];
		   dAcc->check2[i][2]+=indiff_t[i]*up_otvs[i];
	   }
   }
   if(diff_t!=NULL)
	   FreeVector(&fbInfo->tempStack,diff_t);
			 ...
			 DVector aqt1, DVector gqt)
{
   ...
   HMMSet *hset=fbInfo->hset;
   HDFInfo* hdfInfo=&fbInfo->hdfInfo;
   float wght=0;
   float swgt=0;
   ...
   for (j=2;j<N;j++) {
      ...
      for (s=1;s<=fbInfo->S;s++,ste++){
         ...
         if(hset->TVWR){
        	 ApplyWprojection(ste, hdfInfo, t+StartTime, s);
         }
         /* process mixtures */
         for (mx=1;mx<=M;mx++) {
            ...
            case TIEDHS:
               ...
               swgt=wght=MixLogWeight(fbInfo->hset,ste->spdf.tpdf[m]);
               mp=tmRec->mixes[m];
               break;
            case DISCRETEHS:
            ...
            case SHAREDHS:
               m = mx;
               me = ste->spdf.cpdf+m;
               wght = MixLogWeight(fbInfo->hset,me->weight);
               swgt = MixLogWeight(fbInfo->hset,me->sweight);
               mp=me->mpdf;
               break;
            }
            if (swgt>LMINMIX){        /* For this mixture m, if the weight is nonzero... */
               ...
               if (!mmix || (fbInfo->hsKind==DISCRETEHS)){       /*    Don't need the MOutP for 1-mix systems. */
               ...
               } else { /*   Multiple-mixture --> need to work out the occupation probability of the individual mixture. */
                  /*c_jm=log(wght);*/
            	  c_jm=wght;
                  x = initx+c_jm;
                  switch(fbInfo->hsKind){
                  ...
                  x += prob;/*Gaussian component outprob*/
               ...
               if (-x<minFrwdP) {/* mix prune threshold, not all of the mixPdfs associated to this frame are updated */
                  ans2=LAdd(ans2, x);
                  Lr = exp(x);/*Gaussian component posterior*/
                  ...
                  if(fbInfo->uFlags&UPRWGTS){
            		  float dif,temp,chksum;
            		  int k,offset=0;
            		  rwa=((RWtAcc*)(me->rwgt->hook))+local_accindx;
            		  /*store the regular statistics in case of updating static weights normally*/
            		  dif=Lr*abs_mee_acc_scale;
            		  wa->c[m]+=dif;
            		  int *sparse_idx;
            		  Vector posVec;
            		  if(dif!=0){
                    	  short* regwsize=hset->pwidth;
                    	  int _s,c_s=(hset->temp_ctx_size+1)/2;
                    	  int l_s=c_s-1,r_s=(c_s+1)>hset->temp_ctx_size?0:(c_s+1);
                    	  int lctx_loc,rctx_loc;
                    	  switch(hset->rkind){
                    	  case NONE_CTX_REG:
                    	  case TEMP_CTX_REG:
                			  if(l_s>0){
                				  lctx_loc=fbInfo->hdfInfo.ctxLoc[s][t][l_s];
                				  for(k=1;k<=regwsize[s];k++){
                					  temp=dif*fbInfo->hdfInfo.ctxPos[s][lctx_loc][k]*me->rwgt->regparm[offset+k]/me->rwgt->wjmt[l_s];
                					  rwa->hjm[offset+k]+=temp;
                				  }
                				  offset+=regwsize[s];
                			  }
                			  if(c_s>0){
                				  for(k=1;k<=regwsize[s];k++){
                					  temp=dif*fbInfo->hdfInfo.frmPos[s][t][k]*me->rwgt->regparm[offset+k]/me->rwgt->wjmt[c_s];
                					  rwa->hjm[offset+k]+=temp;
                				  }
                				  offset+=regwsize[s];
                			  }
                			  if(r_s>0){
                				  rctx_loc=fbInfo->hdfInfo.ctxLoc[s][t][l_s];
                				  for(k=1;k<=regwsize[s];k++){
                					  temp=dif*fbInfo->hdfInfo.ctxPos[s][rctx_loc][k]*me->rwgt->regparm[offset+k]/me->rwgt->wjmt[r_s];
                					  rwa->hjm[offset+k]+=temp;
                				  }
                				  offset+=regwsize[s];
                			  }
                    		  break;
                    	  case SPAT_CTX_REG:
                	  		  for(_s=1;_s<=regwsize[0];_s++){
                				  for(k=1;k<=regwsize[_s];k++){
                					  temp=dif*fbInfo->hdfInfo.frmPos[_s][t][k]*me->rwgt->regparm[offset+k]/me->rwgt->wjmt[_s];
                					  rwa->hjm[offset+k]+=temp;
                				  }
                				  offset+=regwsize[_s];
                	  		  }
                    	  ...
                    	  case SPTP_CTX_REG:
    						  for(_s=1;_s<=regwsize[0];_s++){
    							  /*left*/
    							  lctx_loc=hdfInfo->ctxLoc[_s][t][l_s];
    							  for(k=1;k<=regwsize[_s];k++){
    								  rwa->hjm[offset+k]+=dif*hdfInfo->ctxPos[_s][lctx_loc][k]*me->rwgt->regparm[offset+k]/me->rwgt->wjmt[(_s-1)*hset->temp_ctx_size+1];
    							  }
    							  offset+=regwsize[_s];
    							  /*center*/
    							  for(k=1;k<=regwsize[_s];k++){
    								  rwa->hjm[offset+k]+=dif*hdfInfo->frmPos[_s][t][k]*me->rwgt->regparm[offset+k]/me->rwgt->wjmt[(_s-1)*hset->temp_ctx_size+2];
    							  }
    							  offset+=regwsize[_s];
    							  /*right*/
    							  rctx_loc=hdfInfo->ctxLoc[_s][t][r_s];
    							  for(k=1;k<=regwsize[_s];k++){
    								  rwa->hjm[offset+k]+=dif*hdfInfo->ctxPos[_s][rctx_loc][k]*me->rwgt->regparm[offset+k]/me->rwgt->wjmt[(_s-1)*hset->temp_ctx_size+3];
    							  }
    							  offset+=regwsize[_s];
    						  }
                    		  break;
                    	  ...
                    	  case TEMP_SPA_REG:
    						  if(l_s>0){
    							  lctx_loc=hdfInfo->ctxLoc[s][t][l_s];
    							  sparse_idx=hset->confuTbl[me->rwgt->spIdx][l_s];
    							  for(k=1;k<=sparse_idx[0];k++){
    								  rwa->hjm[offset+k]+=dif*hdfInfo->ctxPos[s][lctx_loc][sparse_idx[k]]*me->rwgt->regparm[offset+k]/me->rwgt->wjmt[l_s];
    							  }
    							  offset+=sparse_idx[0];
    						  }
    						  if(c_s>0){
    							  sparse_idx=hset->confuTbl[me->rwgt->spIdx][c_s];
    							  for(k=1;k<=sparse_idx[0];k++){
    								  rwa->hjm[offset+k]+=dif*hdfInfo->frmPos[s][t][sparse_idx[k]]*me->rwgt->regparm[offset+k]/me->rwgt->wjmt[c_s];
    							  }
    							  offset+=sparse_idx[0];
    						  }
    						  if(r_s>0){
    							  rctx_loc=hdfInfo->ctxLoc[s][t][r_s];
    							  sparse_idx=hset->confuTbl[me->rwgt->spIdx][r_s];
    							  for(k=1;k<=sparse_idx[0];k++){
    								  rwa->hjm[offset+k]+=dif*hdfInfo->ctxPos[s][rctx_loc][sparse_idx[k]]*me->rwgt->regparm[offset+k]/me->rwgt->wjmt[r_s];
    							  }
    						  }
                    		  break;
                    	  }
   ...
   for (t=1;t<=fbInfo->T;t++) {
      ...
      for (q=fbInfo->aInfo->qLo[t];q<=fbInfo->aInfo->qHi[t];q++){ /* inc accs for each active model */
         ...
         if(t>=tLo&&t<=tHi){
            ...
            if (fbInfo->uFlags&(UPMEANS|UPVARS|UPMIXES|UPXFORM|UPMIXES|UPFPROJ|UPSEMIT|UPRWGTS))
      ...
      /*
       * This is very important when accumulating the stats for the differential w.r.t logistic regression function parameters.
       * Note that only the active mpdf has a nonzero posterior values.
       * Because in the differential calculation, all the component posteriors at this time frame are considered.
       * To define a global index for all Gaussian components so that summation over PARTIAL posteriors can be possible.
      */
      DoAllMixUpdates(t);
...
/*calculate the arc level occupancy*/
void FBLatFirstPass(FBLatInfo *_fbInfo, FileFormat dff, char * datafn, char *datafn2, Lattice *MPECorrLat){
  ...
  /*calculate the backward probability within arc for arc likelihood*/
   SetBetaPlus(); /* Step back through file. */
   /*calculate the forward and backward probabilities on the arc level*/
   {
      ...
      for(a=fbInfo->aInfo->end;a;a=a->prec){ /*Calculate betaPlus .*/
         Acoustic *ac = a->ac;
         LogDouble betaPlus;
         if(!a->follTrans) betaPlus = 0;/*beta(q=Q)=log1=0*/
         else{	  
            betaPlus=LZERO;
         ...
         a->betaPlus = betaPlus;
         if(!a->precTrans)/*no preceding arc found, then the total likelihood is here*/
      ...
      for(a=fbInfo->aInfo->start;a;a=a->foll){
         Acoustic *ac = a->ac;
         LogDouble alpha, occ;
         if(!a->precTrans) alpha = 0;/*alpha(q=1)=log1=0*/
         else{	  
            alpha=LZERO;
         ...
         a->alpha = alpha;
         occ = a->alpha + a->betaPlus - ac->aclike*latProbScale- fbInfo->pr; /*since betaPlus=beta+aclike*latProbScale*/
         ...
         ac->locc = LAdd(ac->locc, occ);/*acoustic(arc) log occupancy*/
      }
         ...
         if(ExactCorrectness){
            ...
            { 
               ...
               for(q=1;q<=fbInfo->Q;q++) {
            	   ...
            	   fbInfo->aInfo->ac[q].mpe_occscale_dif = 0;
               }
...
void ClearOutpCache(HMMSet *hset){
	HMMScanState hss;
	NewHMMScan(hset,&hss);
	while(GoNextStream(&hss,FALSE)){
		((WtAcc *)hss.ste->hook)->time=-1;
	}
	EndHMMScan(&hss);
}
...
void FBLatSecondPass(FBLatInfo *_fbInfo, int num_index, int den_index){
   ...
   if(fbInfo->failedPass){
	   ...
	   if(fbInfo->uFlags!=0)StepForward();
   }

   FBLatClearUp(fbInfo);
   ClearOutpCache(fbInfo->hset);
   /*StartTime += fbInfo->T; /*relates to caching of likelihoods */
   fbInfo->hdfInfo.StartTime=StartTime;
}
                      ...
                      Boolean twoDataFiles)
{
   ...
   if (hset->fMPE||hset->TVWR)
	   InitHDFInfo(&fbInfo->hdfInfo, hset);
   fbInfo->failedPass=FALSE;
}
