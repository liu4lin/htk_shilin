...
/*static LogFloat cSOutP(HMMSet *hset, int s, Observation *x, StreamElem *se, int id)*/
static LogFloat cSOutP(PRecInfo *pri, int s, Observation *x, StreamElem *se, int id, float* aux)
{
   ...
   case SHAREDHS:
      ...
      if (se->nMix==1){     /* Single Mixture Case */
         ...
         if (pre==NULL) {
     	 ...
     	 v_orig=v[v_idx];
     	 if(hset->fMPE){
     		 v=ApplyFprojection((HDFInfo*)hset->hdfInfo,pri->frame);
     	 }
    	 if(hset->TVWR){
    	    ApplyWprojection(se, &pri->hdfInfo, pri->frame, s);
    	 }
         bx=LZERO;                   /* Multi Mixture Case */
         wgtsum=0;
         me=se->spdf.cpdf+1;
         for (m=1; m<=se->nMix; m++,me++) {
        	 wgtsum+=MixWeight(hset, me->weight);
         }
         me=se->spdf.cpdf+1;
         *aux=0;
         for (m=1; m<=se->nMix; m++,me++) {
            wt = MixLogWeight(hset, me->weight);/*this could be a problem, since HVite use weight rather than logWeight*/
            if (wt>LMINRWG) {
               if (me->mpdf->mIdx>0 && me->mpdf->mIdx<=pri->psi->nmp)
                  pre=pri->psi->mPre+me->mpdf->mIdx;
               else pre=NULL;
            ...
            *aux+=MixWeight(hset,me->weight)*me->mpdf->mean[1]/wgtsum;
         }
         *aux-=(v[v_idx]-v_orig);
         *aux=v[v_idx];
      }
...
static LogFloat cPOutP(PRecInfo *pri,Observation *obs,StateInfo *si, TVWRInfo *zi, int id, float* aux)
{
   PreComp *pre;
   LogFloat outp;
   StreamElem *se;
   ...
   if (pre->id!=id) {
      if ((FALSE && psi->mixShared==FALSE) || (psi->hset->hsKind == DISCRETEHS)) {
         ...
         if (S==1 && si->weights==NULL){
             if(pri->hdfInfo.hset->tiedTVWR){
            	 UpdateRegWgts(si->pdf+1, zi, 1);
             }
            outp=cSOutP(pri,1,obs,si->pdf+1,id, aux);
         }
         else {
            outp=0.0;
            se=si->pdf+1;
            ...
            for (s=1;s<=S;s++,se++){
                if(pri->hdfInfo.hset->tiedTVWR){
               	 UpdateRegWgts(se, zi, s);
                }
               outp+=w[s]*cSOutP(pri,s,obs,se,id, aux);
            }
         }
      }
      pre->outp=outp;
                           ...
                           int frame,Align *prev)
{
   ...
   align=(Align*) New(&pri->alignHeap,sizeof(Align));
...
static void StepHMM1(NetNode *node) /* Model internal propagation NBEST */
{
   ...
   for (j=2,res=pri->psi->sBuf+2;j<N;j++,res++) {  /* Emitting states first */
      ...
      if (res->tok.like>pri->genThresh) { /* State pruning */
    	 ...
    	 outp=cPOutP(pri,pri->obs,hmm->svec[j].info, hmm->zvec?hmm->zvec[j].info:NULL, pri->id, &aux);
         res->tok.like+=outp;
         ...
         if (pri->states) {
            ...
            if(res->tok.align){
            	res->tok.align->aux[pri->frame-res->tok.align->frame-1]=aux;
            }
#ifdef PHNALG
            for (n=1;n<res->n;n++) {
            	...
            	if(res->set[n].align){
            		res->set[n].align->aux[pri->frame-res->set[n].align->frame-1]=aux;
            	}
            }
#endif
         }
...
static void StepWord2(NetNode *node) /* Update the path - may be repeated */
{
   ...
   if (node->info.pron==NULL && node->tag==NULL) {
      ...
      /*a critical code ensure the correctness*/
   ...
   for(pth=&tmp;pth!=NULL;pth=pth->chain) {
      ...
      if (align!=NULL) {
	    ...
	    /* didn't handle model that allow 0 frame */
            if (dur<=0 && labid != splabid)
            	HError(8522,"LatFromPaths: Align  have dur<=0");
#endif
            la->lAlign[i].dur=dur;
            la->lAlign[i].like=like;
            la->lAlign[i].aux = al->aux;
            like=al->like;
         }
         if (pr!=NULL) {
#ifdef PHNALG
...
VRecInfo *InitVRecInfo(PSetInfo *psi,int nToks,Boolean models,Boolean states)
{
   ...
   vri->hdfInfo=NULL;
   hset=psi->hset;
   pri->hdfInfo.hset=hset;
   if(hset->fMPE||hset->TVWR){
	   InitHDFInfo(&pri->hdfInfo,hset);
	   vri->hdfInfo=&pri->hdfInfo;
   }
