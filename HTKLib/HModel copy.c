...
typedef enum {   /* Only a character big !! */
   ...
   NUMSTATES, STREAMINFO, VECSIZE, POSINFO, REGWSIZE,/*Note that the old binary format MMF may not be recognizable by this new code, as each element stored in old binary may have different value now*/
...
static ReturnStatus GetOptions(HMMSet *hset, Source *src, Token *tok, int *nState)
{
   ...
   *nState=0; 
   ...
   while (tok->sym == PARMKIND || tok->sym == INVDIAGCOV || 
          ...
          tok->sym == PARENTXFORM || tok->sym == PROJSIZE || tok->sym==REGKIND ||tok->sym==TIEDTVWR||tok->sym==PRIOR||
