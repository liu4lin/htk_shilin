...
void InitVQ(void)
{
   ...
   if (numParm>0){
      ...
      if (GetConfInt(cParm,numParm,"NUMCODE",&i))numCode=i;
   }
...
{
   return TriMatSize(ic)*log(TPI) - CovDet(&gstack, ic);
}
...
static void RecScanTree(VQNode n, DVector pvq, double *sum, Vector v, int size, CovKind ckind){
	if(n==NULL)return;
	double x;
	if(n->vqidx!=0&&n->right==NULL&&n->left==NULL){
		x = VQNodeScore(n,v,size,ckind);
		x = exp(-x/2);
		*sum+=x;
		pvq[n->vqidx]=x;
	}
	RecScanTree(n->right,pvq,sum,v,size,ckind);
    RecScanTree(n->left,pvq,sum,v,size,ckind);
}
/* EXPORT->GetVQ: get vq indices for vectors in fv */
void GetPVQ(VQTable vqTab, int numS, Vector *fv, Vector *vq)
{
   short s,size;
   double x,xl,xr,temp;
   int i,vqsize;
   ...
   Vector v;
   vqsize=numCode;
   DVector pvq=CreateDVector(&gstack,vqsize);
   for (s=1; s<=numS; s++) {
      n = vqTab->tree[s]; v = fv[s];
      size = VectorSize(v);
      if (n==NULL)
      ...
      case binTree:
    	 temp=0;
    	 RecScanTree(n,pvq,&temp,v,size,vqTab->ckind);
         for(i=1;i<=vqsize;i++){
        	 vq[s][i]=pvq[i]/temp;
         }
         break;
